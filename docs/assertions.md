...
| `loweredswitch` | on \| off | `off ⚠️ opt-in` | S2 / switch-model | angr-LoweredSwitchSimplifier | Reconstruct a compiler-lowered comparison cascade (a binary-search if/else tree over one variable) back into a switch with a synthesized BRANCHIND + jump-table (the S2 artifact Ghidra renders switches from). | Set on PER PROGRAM when an enum/char dispatch (e.g. a getopt option parser) renders as a deep if (x == 'p') ... else if (x == 't') ... chain instead of a switch; DESTRUCTIVE as a global default (synthesizes a BRANCHIND/JumpTable upstream never emits, and may convert a hand-written comparison chain). |
