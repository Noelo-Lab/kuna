...
| a Rust Result or Option producer returns an anonymous undefined16 or [u8; 16] container | [`rustadt`](#rustadt) |
| the discriminant and the payload of a returned tagged value are byte offsets into a char array | [`rustadt`](#rustadt) |
| a return site renders as a cast of a value shifted left by 0x40 with no indication of which variant it builds | [`rustadt`](#rustadt) |
| two accesses to the same recovered payload name different union members inside one basic block | [`rustadt`](#rustadt) |
| a recovered two-register return has no type and no field names | [`rustadt`](#rustadt) |
...
### `rustadt` -- off | auto | always, default `off`

- **Symptoms:** a Rust Result or Option producer returns an anonymous undefined16 or [u8; 16] container; the discriminant and the payload of a returned tagged value are byte offsets into a char array; a return site renders as a cast of a value shifted left by 0x40 with no indication of which variant it builds; two accesses to the same recovered payload name different union members inside one basic block; a recovered two-register return has no type and no field names.
- **What it does:** Give the two-register value a rustc ScalarPair return is carried in a NAME and a SHAPE: synthesize the tagged two-variant aggregate, install it as the recovered return type, pin each site's variant from the guard, and emit the variant constructors. `rustabi` (P4) keeps the pair alive as data flow and stops there, so a Rust Result/Option producer renders as a raw undefined16 container with the tag and the payload as byte offsets into it and nothing saying which variant a return path builds. This rule reads the RETURN sites and, when EVERY path's tag is a literal and there are exactly TWO distinct literals, synthesizes `struct { tag; union { Variant0; Variant1 } }` -- union fields are forced to offset 0 by TypeUnion::assignFieldOffsets, which IS a variant overlay, so this is the existing type model used as intended and no type_metatype is added. The type is installed locked between the cleanup rule pool and ActionAssignHigh, which is the only slot where the concatenation has its final shape, the payload half has an inferred type, and the merge phase has not yet built the HighVariable that ActionOutputPrototype reads the recovered return type out of. Each site's union facet is then pinned from the DOMINATING GUARD -- a backward fixpoint over the block graph marks every block whose reachable RETURNs all carry one tag -- and never from ScoreUnionFields, which scores type similarity and cannot see control flow: the two members of a two-variant payload are routinely the same width and metatype, so the trials tie and the winner is trial order. A return site whose value is a single expression renders as `return Variant0(x);` in place of the opaque `return (RustEnum2_...)(ZEXT416(x) << 0x40);`, and a path on which the function writes no payload at all (the unit variant) renders as `Variant0()`. Members are named Variant0/Variant1, NEVER Ok/Err or Some/None: tagged Result is Ok=0/Err=1 and tagged Option is None=0/Some=1 -- opposite polarity, identical codegen, nothing in the shape separating them -- so a guessed name is a coin flip that reads as a finding. Every condition is a refusal and one unreadable path refuses the whole function, because a partially classified union hands the rest back to the scorer. Out of scope by construction: the BRANCHLESS rustc shape (one RETURN, the tag a setCC rather than a literal), three or more variants, and niche-encoded enums (Option<&T> is bit-for-bit a nullable pointer, with no tag at all). At `auto` the rule acts only on an image the loader detected as rustc-produced; at `always` the language test is dropped.
- **When to flip:** Turn on (`auto`) when reading a Rust binary in which `rustabi` has recovered a two-register return and you need to know which variant a path builds and where the payload is. Measured on a 4.9 MB static-PIE rustc binary (4194 functions): with `rustabi auto` in both arms FIVE functions gain a recovered two-variant type, three distinct types are synthesized, five variant constructors and 21 named variant field accesses appear, and no other function changes; against the shipped default (rustabi off too) it is SIX functions and seven constructors. That number is the honest headline and it is small on purpose -- the precondition is a LITERAL tag on every return path, and rustc at -C opt-level=2 overwhelmingly emits the branchless `xor %eax,%eax; setb %al` form instead, which has one RETURN and no literal to group (95 functions in that binary return the pair at all; only 36 have two or more return statements). All six recoveries were hand-checked against their demangled names and all six are correct: core::str::validations::next_code_point, core::ops::range::Range::spec_next_back, hashbrown::raw::capacity_to_buckets, gimli::read::unit::AttributeValue::udata_value, core::slice::memchr::memchr_aligned and core::slice::memchr::memchr all return Option<T>, and in every one Variant0 is the None path and Variant1 the Some path. That is also the naming evidence: all six are Option, so printing Ok/Err would have been wrong on every member of every one. False-positive probe: with `always` (the language gate REMOVED) over 209,943 lines of C/ARM output from grep, sort, faillog, libselinux.so.1, a Cortex-M firmware, /usr/bin/ncat and /usr/bin/htop, ZERO functions change and zero types are synthesized -- the shape does not occur outside Rust in that corpus at all, independently of the gate. Those seven do contain two pair-shaped candidates (both in sort), and both are refused by the two-or-more-returns condition, so the refusal is not vacuous. With `auto` the same seven are byte-identical by construction, since the language fact comes from the analyzer tier. Speed on the Rust binary, interleaved off/auto, 4 pairs: min-of-4 170.90 s -> 170.59 s, -0.18%; the pairs DISAGREE IN SIGN (+0.49, -0.18, -0.17, +0.34), so the real cost is below this machine's measurement floor (budget 5%). DEFAULT OFF this round, for the same reason `rustabi` shipped off: the recovered type is real but the rendering is not yet uniform -- a return site that writes the join Varnode piece by piece renders the construction as field assignments while a single-expression site renders as a constructor call, and unifying them means rewriting return sites after the merge phase. Use `always` only to force the rule on a corpus with no `.comment` record, such as a stage-test bytechunk.
- **Where / provenance:** P5/aggregate-union · kuna · correctness-fix · kuna-rust-tagged-variant-return
- **Example:** `option rustadt auto`
