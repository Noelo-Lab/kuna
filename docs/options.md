...
- **When to flip:** On by default (DIV-102). Keep it on when the same internal callee renders with different arities in one function -- `sub_140008160(0x28)` at one site and `sub_140008160()` at another -- which destroys the dataflow into the callee (a buffer size, an object pointer) exactly where you need it. Most common on MSVC x86-64 around allocator wrappers, whose overflow guard branches on the size the call also takes. It cannot help when the FIRST call to that callee in the function is the one that lost the argument, and it never removes an argument, so an over-recovered site stays over-recovered. Flip back OFF if a call gains an argument no other site passes; the reconciliation is only as good as the witness site it copies.
