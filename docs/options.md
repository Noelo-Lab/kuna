...
| forward goto into the sibling if-arm's body plus a label, where angr emits one compound condition and no goto | [`condjoin`](#condjoin) |
| short-circuit guard cascade split across nested ifs with a goto to the shared body | [`condjoin`](#condjoin) |
| permission or argument-validation check chain renders with a label at the top of the else arm | [`condjoin`](#condjoin) |
...
### `condjoin` -- on | off, default `off`

- **Symptoms:** forward goto into the sibling if-arm's body plus a label, where angr emits one compound condition and no goto; short-circuit guard cascade split across nested ifs with a goto to the shared body; permission or argument-validation check chain renders with a label at the top of the else arm.
- **What it does:** Condition-based join restructuring: let the short-circuit condition fold (ruleBlockOr, in BOTH the region structurer and the CollapseStructure fallback) absorb a CLAUSE BLOCK THAT HOLDS STATEMENTS, instead of declining on Ghidra's BlockBasic::isComplex readability budget (statement count > 2). When a guard cascade's arms reconverge on a shared body, that decline is what forces one of the two edges into the body to be virtualized: kuna emits a forward `goto label_X;` plus a `label_X:` inside the sibling arm where angr emits neither. Relaxing the budget lets the whole cascade fold into one compound condition and the shared body become the fall-through join (lighttpd server.c::main, angr corpus newbury @0x40f696: 1 goto / 1 label -> 0 / 0, 138 -> 131 loc). This is a RELAXATION, not a new transform: the printer already renders a folded condition's second operand under modifiers::COMMA_SEPARATE (emit_basic_block_ops walks every op and emits them comma-separated with the semicolon suppressed), so nothing is dropped, and Ghidra already exercises that path for the 1-extra-statement case. It also cannot reorder anything: new_block_condition only relabels two already-sequenced blocks as the left and right operands of a && / ||, and C short-circuit plus comma sequencing reproduce the CFG's evaluation order exactly, so predicates that call functions (getuid/geteuid/getegid/getgid here) need no purity analysis. Bounded by strict preconditions (expression-shaped ops only: no RETURN, second branch, BRANCHIND, CALLOTHER or no-return call; at most 2 conservatively-scored printed statements and 2 calls per clause block; at most 4 condition leaves and 4 total statements in the folded condition; declined outright when a clause block carries a comment, which the COMMA_SEPARATE path would drop). kuna's COMMA_SEPARATE is the analog of angr's MultiStatementExpression; this option is the analog of angr's MultiStmtExprMode escape hatch in phoenix._match_acyclic_short_circuit_conditions.
- **When to flip:** Set ON when a guard cascade whose arms reconverge on a shared body renders with a forward goto into the sibling arm plus a label (a `goto label_X;` inside an if-arm and `label_X:` at the top of the else-arm), where angr emits a single compound condition and no goto. Typical shape: a SUID/permission or argument-validation check chain where the exceptional path returns and the normal path is the function body. Default OFF because it trades goto count for expression density: the folded operand renders as `(v = geteuid(), v != getuid())` rather than as separate statements, which is more compact but denser than the goto form. Leave OFF to keep the statement-per-line rendering; nothing else changes (output is byte-identical with it off).
- **Where / provenance:** P8/goto-quality · angr · structure-recovery · angr-newburry-main-57ccb1
- **Example:** `option condjoin on`
