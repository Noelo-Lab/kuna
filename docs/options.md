...
| second condition block assigns one value before its test and the fold is declined | [`condfold`](#condfold) |
...
- **Symptoms:** crossing goto from the else arm back into the then arm's shared clause; nested if where angr emits one compound short-circuit condition; second condition block assigns one value before its test and the fold is declined.
- **What it does:** Extend the short-circuit condition fold (ruleBlockOr / _match_acyclic_short_circuit_conditions) so it also fires when the SIBLING condition block carries a bounded, side-effect-ordered prefix. Ghidra declines the A || B fold whenever the sibling is complex (BlockBasic::isComplex: more than two statements), so one spill or address computation parked in front of the second test costs a crossing goto back into the first arm's clause (coreutils tr build_spec_list: `result_idx = &v10; if (es_match(es,v12,'=')) goto label_40242e;`). With this on, a complex sibling is accepted provided it is a BlockCopy of ONE basic block whose printed prefix is bounded (<=1 statement-root call, and <=5 statements at `on` / <=9 at `wide`), branch-free, and comment-free; the prefix then renders inside the &&/|| operand as a C comma expression, which the printer already supports. The fold moves no p-code -- it re-parents two existing structuring nodes -- and C short-circuit plus comma sequencing preserves the original execution paths and order. Applied at BOTH structuring paths (region_structurer try_block_or and the CollapseStructure rule_block_or fallback). Port of the angr Phoenix MultiStatementExpression relaxation (_should_use_multistmtexprs / _build_multistatementexpr_statements) under its default MAX_ONE_CALL policy; `on` uses angr's statement threshold (5) and `wide` raises only the statement cap to 9, because kuna's printed-statement granularity is finer than angr's AIL for the same block (argument recovery and address-tied copy shadows split one AIL statement into several printed ones). NOTE the call cap is not an exact match for angr's MAX_ONE_CALL: condfold_eligible mirrors the printer's skip rules, and the implied-output skip runs before the call test, so a call whose result is inlined into the sibling's own condition is not charged. A folded operand can therefore render more than one call (measured: 1 of 46 new call-bearing operands -- mv -O2 copy_internal at `wide` renders cached_umask(...) counted and fchmod(...) not counted). The cap is a readability bound, not a soundness bound, so the looser count cannot produce wrong C.
- **When to flip:** kuna emits a crossing `if (cond) goto label_X;` from the else arm back into the then arm's clause, or a nested if where angr short-circuits into `A || B` / `A && B`, because the second condition block also assigns something (a spill, a pointer setup, a call whose result is the test). Default off: byte-identical to upstream (the eligibility precompute is skipped and both gate disjuncts are dead). Try `on` first (angr's statement threshold); go to `wide` when the sibling is a kuna-noisy block (copy shadows, unrecovered call arguments) that `on` still declines -- on tr build_spec_list `on` removes the crossing goto and `wide` additionally folds the third term, 5 gotos -> 3. Flip back off when you prefer a nested if to a comma expression in the condition, or when the folded operand reads as too dense -- note the <=1 call cap counts only calls printed as their own comma-chain element, so an operand can occasionally render two calls when one is inlined into the test (readability only; it cannot make the C wrong).
