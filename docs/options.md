...
- **When to flip:** Turn on (`auto`) when reading a Rust binary in which `rustabi` has recovered a two-register return and you need to know which variant a path builds and where the payload is. Measured on a 4.9 MB static-PIE rustc binary (4194 functions, `rustabi auto` in both arms): FIVE functions gain a recovered two-variant type and three distinct types are synthesized; no other function changes. That number is the honest headline -- the precondition is a LITERAL tag on every return path, and rustc at -C opt-level=2 overwhelmingly emits the branchless `xor %eax,%eax; setb %al` form instead, which has one RETURN and no literal to group. All five recoveries were hand-checked against their demangled names and all five are correct: core::str::validations::next_code_point, core::ops::range::Range::spec_next_back, hashbrown::raw::capacity_to_buckets, gimli::read::unit::AttributeValue::udata_value and core::slice::memchr::memchr_aligned all return Option<T>, and in every one Variant0 is the None path and Variant1 the Some path. That is also the naming evidence: all five are Option, so printing Ok/Err would have been wrong on every member of every one. False-positive probe: with `always` (the language gate removed) over grep, sort, faillog, libselinux.so.1, an ARM Cortex-M firmware, /usr/bin/ncat and /usr/bin/htop -- 209k lines of C output including two OpenSSL-linked binaries -- ZERO functions change and zero types are synthesized, so the shape does not occur in C at all in that corpus. With `auto` those seven are byte-identical by construction, since the language fact comes from the analyzer tier. Speed on the same Rust binary, interleaved off/auto: see the PR. DEFAULT OFF this round, for the same reason `rustabi` shipped off: the recovered type is real but the rendering is not yet uniform -- a return site that writes the join Varnode piece by piece renders the construction as field assignments while a single-expression site renders as a constructor call, and unifying them means rewriting return sites after the merge phase. Use `always` only to force the rule on a corpus with no `.comment` record, such as a stage-test bytechunk.
