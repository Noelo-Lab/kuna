...
| both arms of a Rust match on a Result render the same field_0x8 payload | [`variantguard`](#variantguard) |
| a recovered Rust enum names no variant even though the binary has full debug info | [`variantguard`](#variantguard) |
| Ok and Err are recovered as a layout but never appear in the emitted C | [`variantguard`](#variantguard) |
| a tagged enum's payload is spelled by byte offset on every access | [`variantguard`](#variantguard) |
| a variant constructor that stores a literal discriminant does not name the variant it builds | [`variantguard`](#variantguard) |
...
### `variantguard` -- on | off, default `off`

- **Symptoms:** both arms of a Rust match on a Result render the same field_0x8 payload; a recovered Rust enum names no variant even though the binary has full debug info; Ok and Err are recovered as a layout but never appear in the emitted C; a tagged enum's payload is spelled by byte offset on every access; a variant constructor that stores a literal discriminant does not name the variant it builds.
- **What it does:** PROPOSAL, DEFAULT-OFF AND NOT IN ANY PRESET: the MEMORY-OBJECT guard direction is KNOWN-UNSOUND and can print a confidently wrong variant name -- a tag read hoisted above a clobbering call with the branch below it (witness hoist_read/hoist_xblock), because a CBRANCH sits last in its block but its condition Varnode need not, so the block-level kill is re-applied as a stale edge constraint; closing it needs reaching definitions plus an escape analysis over the object, not positional CFG reasoning. See docs/features/variantguard/analysis.md. Pick a recovered Rust enum's union facet from the DWARF discriminant and the branch or store that fixed it, instead of from ScoreUnionFields, and spell the DW_TAG_variant name wherever that proof exists. dwarfvariants (DIV-87) installs a tagged enum as struct { tag; union payload; } and records the exact geometry -- the discriminant's offset and width, and per variant its source name, its DW_AT_discr_value and its payload struct -- then refuses to name any variant where two of them overlay one byte range, because a union member selects itself by OFFSET and the discriminant is never consulted, so Ok.__0 and Err.__0 tie in the scorer and the winner is trial order. Every Result<T,E> is that case. This is the analysis that side table was recorded for. Two seed kinds, both anchored on a Varnode whose RECOVERED TYPE is the enum or a pointer to it, so no codegen shape can trigger it: a CONSUMER guard (a CBRANCH testing the tag bytes for equality, possibly masked, truncated, zero-extended or negated, whose predicate is evaluated over the enum's discriminant domain -- every DW_AT_discr_value plus the DEFAULT niche variant, which is admitted only when a value outside the claimed set satisfies the edge, counted exactly), and a PRODUCER store (a constant written over the tag bytes, which selects the facet facet_for_discr names). Both are subject to ONE kill discipline, because every defect this pass shipped was a fact about a discriminant applied where it no longer held: a guard proves what the object was AT THE GUARD and a store proves what it becomes, and neither survives an EVENT -- a store over the tag bytes, a call (a callee handed the pointer may store through it), or any store the pass cannot attribute to the object (it may alias it). A VALUE object has no events, because an SSA value cannot be clobbered. Within a block the kill is positional, so a read above a clobber keeps the guard and one below it does not; a guard's own CBRANCH is its block's last op, so a re-test after a clobber still constrains the edge. The two facts are INTERSECTED at the pin and a disagreement REFUSES -- neither outranks the other, because a revision that gave a singleton guard region precedence named a write that builds Err, inside an Ok-guarded block, Ok, one line below its own tag = 1. Otherwise: a guard is evidence about the value an execution is carrying, so it colours every op its edge reaches, reads included; a store is evidence about what the object BECOMES, so it colours WRITES ONLY -- a store into the object and the pointer arithmetic that feeds one and nothing else -- read positionally off the nearest constant tag store above or below it in its own block, killed by any intervening non-constant tag store or call, and refused when the two sides disagree. An earlier revision propagated the store backwards over whole blocks with no ordering test and no kill, which named an UNGUARDED read of the caller's value after the variant a later store was about to write; the control is decisive, because the same function clobbering with Ok instead of Err gave the SAME instruction the opposite name. A forward may-analysis propagates the guard edge constraints; a block is a region only where its set is a singleton. A backward walk over def-use then reaches the value a compiler HOISTED above the branch -- `match r { Ok(v) => v, Err(e) => e + 100 }` at -C opt-level=1 computes e + 100 before the branch and selects it with a cmov -- by reading a MULTIEQUAL input slot's context as its own IN-EDGE rather than the phi block's, which also makes the walk acyclic and therefore exact. The proof is recorded on a Funcdata side channel keyed by op time AND by instruction address, because the ResolvedUnion lock does not survive resolve_in_flow's address-based materialization or ActionSetCasts' spliced PTRSUB, both of which build a fresh unlocked resolution and both of which create the very ops the printer descends with; the printer spells the variant name only where that proof matches the member it resolved. SCOPE: only a layout with a SUPPRESSED payload facet is considered at all, so an enum dwarfvariants already names -- Option<T>, enum List { Cons, Nil }, an enum whose payloads occupy disjoint ranges -- is skipped before any analysis runs and cannot be disturbed. Every access is attributed to an object ROOT, so a second Result in the same function is not steered by the first one's guard. A single use whose own context is still ambiguous BLOCKS the conclusion rather than merely failing to narrow it, which is the difference between a proof and a majority vote. LOSS: a branchless producer names nothing (rustc at -O1 computes a Result's discriminant as (x < 0xb) rather than storing a literal, so put_res and r16 in the committed overlay fixture stay on the offset spelling); a switch or jump-table dispatch on the tag is not read as a seed, only equality-shaped conditions are; a producer store names no READ at all, not even one below it, and does not reach out of its own block, so only a guard ever names a read and only until the first event; ANY call kills a memory object's guard with no attempt to prove the callee cannot reach it, which is the largest measured cost (a std-linked -g witness goes from 5 functions / 17 labels to 4 / 11, the loss being reads below a call in drop glue and a recursive tree walk) and refining it needs an escape analysis; a read consumed BOTH by the discriminant test and inside one arm is refused, which on a NICHE encoding is the same bytes (enum Tree { Leaf(i64), Node(Box,Box) } stores the tag in Node's first box), so its drop glue names the arm's read and leaves the guard's read on the offset spelling; the facet's own interned TYPE name is never substituted, so a cast still prints Result<u64, u64>::field_0x8; and the FIELD inside a named facet keeps dwarfvariants' own per-field suppression (Multi's P.field_0x8).
- **When to flip:** DO NOT turn on for production output: the memory-object guard is known-unsound (see the summary). On, it names the arms of a Rust match: `match r16(x) { Ok(v) => v, Err(e) => e + 100 }` renders v2.payload.Ok and v2.payload.Err instead of v2.payload.field_0x8 twice, and a constructor that stores a literal tag names the variant it builds ((rethidden->payload).P.a = x instead of (rethidden->payload).field_0x4.a = x). Inert without full debug info: it reads the dwarfvariants side table, which is empty unless the binary carries DW_TAG_variant_part DIEs, so a C binary or a stripped Rust binary is untouched by construction. Flip off to restore the DIV-87 rendering, where an overlaying enum's payload is always spelled by offset. Turning dwarfvariants off suppresses this too, because the side table is then never filled.
- **Where / provenance:** P5/aggregate-union · kuna · correctness-fix · kuna-dwarfvariants-dominating-guard
- **Example:** `option variantguard on`
