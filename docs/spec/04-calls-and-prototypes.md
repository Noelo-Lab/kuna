...
- **What the caller does through a pointer disagrees with the pointee.** A
  ...
  *)&a0[4].field_0x8` for a callee's 16-byte view, and sort's 32-byte records
  ...
  printed bzip2's field write `*(unsigned long *)(a0 + 0x5c) = 0x100` as eight
  byte stores, betaflight's `1.0f` in a 0x14-byte record as four, and gzip's
  `".tar"` suffix as five (the fixture `protoorder_narrowvote_x86_64`). A
