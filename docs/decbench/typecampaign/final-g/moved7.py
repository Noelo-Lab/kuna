"""moved.csv (round F -> round G) and report-slices.md (base/B/C/D/E/F/G per project x opt)."""
...
with open("/home/mahaloz/kwt/_final-g/moved.csv", "w", newline="") as f:
    w = csv.writer(f); w.writerow(["slice", "function", "round_f", "round_g", "delta"])
    for k in sorted(both):
        if abs(V["roundG"][k] - V["roundF"][k]) > 1e-9:
            w.writerow([k[0], k[1], round(V["roundF"][k], 4), round(V["roundG"][k], 4),
                        round(V["roundG"][k] - V["roundF"][k], 4)])
...
lines = ["| project | opt | n | perfect base | perfect B | perfect C | perfect D | perfect E | perfect F | perfect G | mean base | mean B | mean C | mean D | mean E | mean F | mean G |",
         "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|"]
...
Path("/home/mahaloz/kwt/_final-g/report-slices.md").write_text("\n".join(lines) + "\n")
print("\n".join(lines))
