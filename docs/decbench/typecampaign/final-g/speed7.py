"""Interleaved min-of-N whole-binary decompile-all timing: baseline, round E, round F, round G."""
import json
import os
import statistics
import subprocess
...
ARMS = ["base", "roundE", "roundF", "roundG"]
N = int(sys.argv[1]) if len(sys.argv) > 1 else 11
outf = sys.argv[2] if len(sys.argv) > 2 else "/home/mahaloz/kwt/_final-g/speed.json"
...
for name in which:
    ...
    out[name] = r
    print(name, {lab: r[lab]["min_ms"] for lab in ARMS}, "G/F", r["delta_min_pct_roundG_vs_roundF"],
          "G/E", r["delta_min_pct_roundG_vs_roundE"], "G/base", r["delta_min_pct_vs_base_roundG"],
          "G/F median", r["delta_median_pct_roundG_vs_roundF"], os.getloadavg(), flush=True)
