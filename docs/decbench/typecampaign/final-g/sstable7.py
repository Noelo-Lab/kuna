"""structscore E -> F comparison table (identical instrument, both binaries re-run)."""
import json, sys
from pathlib import Path
C = Path("/home/mahaloz/kwt/_final-g/ss"); D = Path("/home/mahaloz/kwt/_final-g/ss")
rows = []
pool = {"B": [0, 0], "C": [0, 0], "D": [0, 0]}   # points_possible, points_lost per opt handled below
acc = {}
print("| opt | binary | TRex mean 0-6 F -> **G** | mean 0-5 | `is_c_struct` passed | `c_primitive` passed | params typed as a struct / GT | layout F1 (filler counted) | nesting tp/pred/gt |")
print("|---|---|---|---|---|---|---|---|---|")
tot = {k: {"pp": 0, "pl": 0, "pp5": 0, "pl5": 0} for k in ("C", "D")}
for opt in ("O0", "O2"):
    for b in ("fmt", "ls", "sort", "du"):
        try:
            c = json.loads((C / f"f-{opt}-{b}.json").read_text())[0]
            d = json.loads((D / f"g-{opt}-{b}.json").read_text())[0]
