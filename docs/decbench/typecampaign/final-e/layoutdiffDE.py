...
def main():
    ...
    pd, ld = arm("/home/mahaloz/kwt/_final-d/kuna", binary)
    pe, le = arm("/home/mahaloz/kwt/_final-e/kuna", binary)
    rd, re_ = LD.rows(binary, functions, pd, ld), LD.rows(binary, functions, pe, le)
    print(f"params typed as a struct with a header layout: D {len(rd)}  E {len(re_)}")
        ...
        if fn:
            for v in fn.get("variables") or []:
                if v.get("arg_index") == k[1]:
                    etype = v.get("type")
        print(f"  {k[0]}(arg {k[1]}): D {a} | E {b} | E type {etype!r}")
