"""Round-E aggregate: baseline vs round B vs round C vs round D vs round E, per slice, per GT class, rivals."""
import collections
import json
from pathlib import Path

OUT = Path("/home/mahaloz/kwt/_final-e")
...
SETS = {"base": B / "sweep-base/rows.json", "roundB": C / "sweep-bctl/rows.json",
        ...
        "roundE": OUT / "sweep-e/rows.json", "roundDctl": OUT / "sweep-dctl/rows.json"}
ARMS = ["base", "roundB", "roundC", "roundD", "roundE"]
...
def main():
    ...
    for a, b in (("roundD", "roundE"), ("roundC", "roundE"), ("roundB", "roundE"), ("base", "roundE"), ("roundD", "roundDctl")):
    ...
    both = set(V["roundD"]) & set(V["roundE"])
    pw = collections.defaultdict(lambda: [0, 0])
    for k in both:
        p = k[0].split("::")[0]
        pw[p][0] += V["roundE"][k] > V["roundD"][k] + 1e-9
        pw[p][1] += V["roundE"][k] < V["roundD"][k] - 1e-9
    rep["per_project_moves_D_to_E"] = {p: {"improved": a, "worse": b} for p, (a, b) in sorted(pw.items())}
    rep["worse_D_to_E"] = [[round(V["roundE"][k] - V["roundD"][k], 4), k[0], k[1],
                            round(V["roundD"][k], 4), round(V["roundE"][k], 4)]
                           for k in sorted(both, key=lambda k: V["roundE"][k] - V["roundD"][k])
                           if V["roundE"][k] < V["roundD"][k] - 1e-9]
    rep["best_D_to_E"] = [[round(V["roundE"][k] - V["roundD"][k], 4), k[0], k[1]]
                          for k in sorted(both, key=lambda k: V["roundD"][k] - V["roundE"][k])[:30]]
    ...
    print(json.dumps({k: rep[k] for k in ("headline", "moves_roundD_to_roundE", "moves_roundD_to_roundDctl", "moves_base_to_roundE", "per_project_moves_D_to_E", "worse_D_to_E")
