"""moved.csv (round D -> round E) and report-slices.md (base/B/C/D/E per project x opt)."""
...
with open("/home/mahaloz/kwt/_final-e/moved.csv", "w", newline="") as f:
    w = csv.writer(f); w.writerow(["slice", "function", "round_d", "round_e", "delta"])
    for k in sorted(both):
        if abs(V["roundE"][k] - V["roundD"][k]) > 1e-9:
            w.writerow([k[0], k[1], round(V["roundD"][k], 4), round(V["roundE"][k], 4),
                        round(V["roundE"][k] - V["roundD"][k], 4)])
...
lines = ["| project | opt | n | perfect base | perfect B | perfect C | perfect D | perfect E | mean base | mean B | mean C | mean D | mean E |",
         "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|"]
...
Path("/home/mahaloz/kwt/_final-e/report-slices.md").write_text("\n".join(lines) + "\n")
print("\n".join(lines))
