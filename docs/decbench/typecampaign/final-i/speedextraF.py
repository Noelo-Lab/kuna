"""Interleaved min-of-N on the three binaries #711's review flagged: round F (before #711) vs round G vs final."""
...
ARMS = ["roundF", "roundG", "roundI"]
N = int(sys.argv[1]) if len(sys.argv) > 1 else 15
outf = sys.argv[2] if len(sys.argv) > 2 else "/home/mahaloz/kwt/_final-i/speed-extraF.json"
...
for name in which:
    ...
    r["delta_min_pct_G_vs_F"] = round((r["roundG"]["min_ms"] - r["roundF"]["min_ms"]) / r["roundF"]["min_ms"] * 100, 2)
    out[name] = r
    print(name, {lab: r[lab]["min_ms"] for lab in ARMS}, "I/G min", r["delta_min_pct"], "med", r["delta_median_pct"], "I/F min", r["delta_min_pct_I_vs_F"], "G/F min", r["delta_min_pct_G_vs_F"], os.getloadavg(), flush=True)
