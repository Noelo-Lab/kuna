...
| C14 | `layoutrecall` → `structmerge siblings` — two functions measuring one record get the union, not a second `struct_N` | C | open, gates green, **no review** | /home/mahaloz/kwt/layoutrecall (feat/layoutrecall) | #715 | on the round-G tree: layout precision .8713 → .8730 and recall .0932 → .0960 on the eight layout builds; `type_match` 1,522 either way; default would be `off` | review and land |
