...
| A3 | protoorder | A | draft #669: round 4 CHANGES → REDESIGN round 5 (types-only, never change arity) running | /home/mahaloz/kwt/protoorder | #669 | locking design: +105 perfect but FABRICATES params (x86-64 phantom 281→374, 155 fns gain arg rows; ARM 630 fns / 730 over-arity) → fails the user's accuracy rule on arity; types-only redesign in flight | land only if arity counters are 0 |
