...
| S7 | final re-measure, round F | setup | done 2026-09-23 | /home/mahaloz/kwt/_final-f | results PR | 1,522 perfect / .3553; 581 improved / 6 worse; `ptr_char` 28.9% → 34.6%; stack storage 53.4% → 57.6%; TRex O0 4.129 → 4.420; instruments byte-identical E → F; speed −3.9…+2.2% vs round E | `results.md` §Round F |
| C13 | `calleevote` — a parameter takes the type every caller passes | C | merged 9e07ab93 (default `fields`) | — | #711 | on the final tree: `off` → `fields` 1,575 → 1,609 perfect, 101 up / 3 worse (gnulib `xmemdup`); the whole of goal 3's movement and +322 credited functions under decbench#93 | incremental redo (kmod -O2-noinline speed) |
| A11 | `libcwiden` — 333 more measured libc signatures + `clearStackPlaceholders` | A | merged 1c57f06b (strict, ungated) | — | #714 | 1,567 → 1,582 perfect, 202 up / 4 worse (four `_Bool` slots become `char`); phantom trailing arguments 149 → 0 over 12 binaries | `boolbyte` follow-up |
| A10-on | `passthrough` default on, + `returnuncomputed` / `varargtail` refusals | A | merged 10a0db23 (default **on**) | — | #716 | 1,582 → 1,609 perfect, 111 up / **0 worse**; the only lever that moves -O2 (80 → 89) and -O2-noinline (398 → 416); 4,107/4,346 gained parameters DWARF-confirmed over 2 corpora, 0 contradicted | — |
| C14 | `layoutrecall` → `structmerge siblings` — two functions measuring one record get the union, not a second `struct_N` | C | merged dbe854ba (default **off**) | — | #715 | default output identical to round G on typesweep, castbench and layout; option arm on the final tree: layout precision .8713 → .8730, recall .0932 → .0960 | — |
| S8 | final re-measure, round G | setup | done 2026-09-23 | /home/mahaloz/kwt/_final-g | results PR | 1,609 perfect / .3688; 457 improved / 7 worse; `ptr_char` 34.6% → 38.0% (passes binja); 1st of five at EVERY optimization level; TRex O0 4.420 → 4.475 and O2 1.918 → 1.952; #93 crediting 1,781 → 2,181 | `results.md` §Round G |
| S9 | final re-measure, rounds H + I | setup | done 2026-09-24 | /home/mahaloz/kwt/_final-i | results PR | 1,615 perfect / .3697; G → final 69 improved / 0 worse (all #719; the three cast levers move 0 of 10,748 values); casts 45,126 → 38,602 (1.193× → 1.021× IDA; baseline 40,583, 1.073×), 1,353 functions fewer / 1 more; TRex 4.4770 / 1.9597; goal 2 and layout identical to G; #93 credited 2,188 | `results.md` §Campaign summary, §Round H, §Round I |
| C13p | `calleevoteperf` — the callee redo gets a 5% budget instead of a flat 32-line refusal | C | merged 22df4282 (ungated) | — | #719 | 1,609 → 1,615 perfect, 69 up / 0 down; casts −87; kmod / crontab / dpkg-divert -O2-noinline −0.7 / −1.2 / −0.3% vs round G | incremental redo (substrate change) |
| X1 | `castimplied` — a cast C's own conversion performs is not printed | cast | merged 5458b7ab (default on) | — | #721 | casts −1,366 on its step (570 functions fewer, 0 more); typesweep identical | — |
| X2 | `castarith` — pointer + whole elements prints as `((T *)p)[k]` | cast | merged 52202840 (default on) | — | #722 | casts −4,970 (952 fewer, 0 more), the largest cast lever; typesweep identical | variable / scaled indexes, pointer difference |
| X3 | `castsign` — a stack local compared only signed is declared signed | cast | merged b3878d32 (default on) | — | #724 | casts −101 (41 fewer, 0 more); typesweep identical | — |
| X4 | `globalref` (castglobals) — a constant address used as a pointer prints as `&dat_<addr>` | cast | open, review APPROVED | /home/mahaloz/kwt/castglobals (feat/castglobals) | #723 | casts 45,039 → 43,914 (486 fewer, 0 more); typesweep 1,615 = 1,615 | land |
| C15 | `structheadless` (structdeep) — a parameter read only past offset 0 is a record when every caller is direct | C | open, review APPROVED, default **off** | /home/mahaloz/kwt/structdeep (feat/structdeep) | #720 | option on: casts −3,118, layout recall .0932 → .1131, nesting F1 .0036 → .0072; precision .8713 → .8298, typesweep 1 up / 2 down | land opt-in |
| A12 | `callbacktype` — a callback takes its libc slot's prototype | A | open, review CHANGES | /home/mahaloz/kwt/callbacktype (feat/callbacktype) | #718 | not re-measured | the three required review changes |
