"""Round-F aggregate: baseline vs round B/C/D/E vs round F, per slice, per GT class, rivals."""
import collections
import json
from pathlib import Path

OUT = Path("/home/mahaloz/kwt/_final-f")
...
SETS = {"base": B / "sweep-base/rows.json", "roundB": C / "sweep-bctl/rows.json",
        ...
        "roundE": E / "sweep-e/rows.json",
        "roundF": OUT / "sweep-f/rows.json", "roundEctl": OUT / "sweep-ectl/rows.json"}
ARMS = ["base", "roundB", "roundC", "roundD", "roundE", "roundF"]
...
def main():
    ...
    for a, b in (("roundE", "roundF"), ("roundD", "roundF"), ("roundC", "roundF"), ("roundB", "roundF"), ("base", "roundF"), ("roundE", "roundEctl")):
    ...
    for k in both:
        p = k[0].split("::")[0]
        pw[p][0] += V["roundF"][k] > V["roundE"][k] + 1e-9
        pw[p][1] += V["roundF"][k] < V["roundE"][k] - 1e-9
    rep["per_project_moves_E_to_F"] = {p: {"improved": a, "worse": b} for p, (a, b) in sorted(pw.items())}
    rep["worse_E_to_F"] = [[round(V["roundF"][k] - V["roundE"][k], 4), k[0], k[1],
                            round(V["roundE"][k], 4), round(V["roundF"][k], 4)]
                           for k in sorted(both, key=lambda k: V["roundF"][k] - V["roundE"][k])
                           if V["roundF"][k] < V["roundE"][k] - 1e-9]
    rep["best_E_to_F"] = [[round(V["roundF"][k] - V["roundE"][k], 4), k[0], k[1]]
                          for k in sorted(both, key=lambda k: V["roundE"][k] - V["roundF"][k])[:40]]
    ...
    for r, rv in rv_vals.items():
        ...
        for opt in ("O0", "O2", "O2-noinline"):
            ...
            row["by_opt"][opt] = {"rival": summarize(sub),
                                  ...
                                  "kuna_roundE": summarize({k: V["roundE"][k] for k in subc})}
        rep["rivals"][r] = row
    ...
    print(json.dumps({k: rep[k] for k in ("headline", "moves_roundE_to_roundF", "moves_roundE_to_roundEctl", "moves_base_to_roundF", "per_project_moves_E_to_F", "worse_E_to_F")
