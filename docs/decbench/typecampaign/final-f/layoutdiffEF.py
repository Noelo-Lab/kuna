...
sys.path.insert(0, "/home/mahaloz/kwt/_final-f/tools")
...
def main():
    ...
    pd, ld = arm("/home/mahaloz/kwt/_final-e/kuna", binary)
    pe, le = arm("/home/mahaloz/kwt/_final-f/kuna", binary)
    rd, re_ = LD.rows(binary, functions, pd, ld), LD.rows(binary, functions, pe, le)
    print(f"params typed as a struct with a header layout: E {len(rd)}  F {len(re_)}")
        ...
        if fn:
            for v in fn.get("variables") or []:
                if v.get("arg_index") == k[1]:
                    etype = v.get("type")
        print(f"  {k[0]}(arg {k[1]}): E {a} | F {b} | F type {etype!r}")
