...
| `decompiler/cpp/architecture.{hh,cc}` (Rust: `infra/architecture.rs`) | (kuna) DIV-12: additive `noreturn_extern_match` flag (default **on** in ctor + reset) + `"noreturn_externmatch"` `on_off!` dispatch arm. Gates the name-matched-extern no-return flow check (`kuna_noreturn_externmatch`). Clean 0/675 ablation ⇒ no vendored behavior change at the default. |
| `decompiler/cpp/funcdata.cc` / flow query seam (Rust: `infra/decompile_drive.rs`) | (kuna) DIV-12: `ArchFlowEnv::query_call_no_return` also reports no-return for a callee whose name matches the vendored ELF known-no-return list when `noreturn_extern_match` is set (one gated `if`). Reaches the ET_REL `.o` undefined extern the address-keyed scan misses. |
| `decompiler/cpp/options.cc` (Rust: `p0_knowledge/options.rs`) | (kuna) DIV-12: registers the `noreturn_externmatch` option name in `KUNA_OPTION_NAMES`. Additive. |
