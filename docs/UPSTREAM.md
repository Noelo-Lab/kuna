...
Only two upstream subtrees are still vendored (the C++ decompiler source and its unittests
were ported to Rust and removed — `tools/sync_upstream.py` no longer maps them):
...
| `decompiler/cpp/architecture.{hh,cc}` (Rust: `infra/architecture.rs`) | (kuna) DIV-13: additive `noreturn_extern_match` flag (default **on** in ctor + reset) + `"noreturn_externmatch"` `on_off!` dispatch arm. Gates the name-matched-extern no-return flow check (`kuna_noreturn_externmatch`). Clean 0/675 ablation ⇒ no vendored behavior change at the default. |
| `decompiler/cpp/funcdata.cc` / flow query seam (Rust: `infra/decompile_drive.rs`) | (kuna) DIV-13: `ArchFlowEnv::query_call_no_return` also reports no-return for a callee whose name matches the vendored ELF known-no-return list when `noreturn_extern_match` is set (one gated `if`). Reaches the ET_REL `.o` undefined extern the address-keyed scan misses. |
| `decompiler/cpp/options.cc` (Rust: `p0_knowledge/options.rs`) | (kuna) DIV-13: registers the `noreturn_externmatch` option name in `KUNA_OPTION_NAMES`. Additive. |
...
| `decompiler/cpp/jumptable.{hh,cc}`, `decompiler/cpp/architecture.{hh,cc}`, `decompiler/cpp/options.cc`, `decompiler/cpp/kuna_stages.cc` | angr `test_decompiling_missing_function_call`: `JumpBasic::kuna_try_guard_bound_table` (+ helpers `scan_guard_tree`/`build_offset_map`/`eval_guard_expr`) bounds a LOAD-table jumptable by an out-of-band CBRANCH range guard the guard analysis could not, by EVALUATING the guard boolean as a function of the index (re-bind to `[0, N)`); called from `recover_model_basic` after the modulo extension. Arch flag `switch_guard_bound` (decl + struct-init + reset + `set_kuna_option` dispatch + ArchSeam copy in `build_arch_handle`); `seams.rs` ArchSeam field + init; `options.rs` `KUNA_OPTION_NAMES`; `stages.toml` settable + surface rows; new module `kuna_switchguardbound.rs` (ELEM 4022, option `switchguardbound`, default-off opt-in — ablation 0/675, speed +34% so forced opt-in) |
...
| `decompiler/cpp/architecture.{hh,cc}` (Rust: `infra/architecture.rs` + `substrate/seams.rs`), `decompiler/cpp/printc.cc` (Rust: `s9_emit/printc.rs`), `decompiler/cpp/options.cc` (Rust: `p0_knowledge/options.rs`), `decompiler/cpp/kuna_stages.cc` (Rust: `stages.toml`) | DIV-7 angr duplicate-declaration collapse: flag `dedup_var_decls` (decl + default-on `resetDefaults` init + `ArchSeam` copy), `set_kuna_option` dispatch arm, `KUNA_OPTION_NAMES` entry, gated skip in `emit_local_var_decls`, settableTable row — all `(kuna)`, `option dedupvardecls` (ElementId 4091); new file `kuna_dedupvardecls.rs`; DIV-7 default-on (ablation: 0/675, speed +0.14%; `docs/divergences.md`) |
| `decompiler/cpp/loadimage_bfd.{cc,hh}` (kuna analog: `kuna-analysis/src/loadimage_object.rs`), `decompiler/cpp/architecture.{hh,cc}`, `decompiler/cpp/options.cc` | DIV-8 angr ET_REL loader: `ObjectLoadImage::from_bytes` takes a new `from_relocatable` path for `ET_REL` objects (no PT_LOAD), implemented in **new** kuna file `kuna-analysis/src/s1_loader/elf_reloc.rs` (section layout above 0x400000 + `.rela.*` relocation application + symbol rebasing/externs). `set_kuna_option` gains a `relocobjects` arm; `options.rs` gains the option name + `RELOC_OBJECTS_ENV`; `kuna-cli/src/decompile.rs` bridges the option to the loader env var. `stages.toml` settableTable + `kuna-catalog.xml` provenance counts (5→6 angr, 2→3 structure-recovery). All `(kuna)`, `option relocobjects`, DIV-8 default-on; linked-ELF path byte-identical. No new ElementId (loader capability, not a PcodeOp/element). Also corrected a pre-existing stale `xunknown4` oracle assertion in `verify_w10_proto_unlock.rs` (a DIV-6 leftover blocking `make rust-test`). |
| `architecture.{hh,cc}` (`infra/architecture.rs`), `options.cc` (`p0_knowledge/options.rs`), `kuna_console.cc` (`kuna-console/src/kuna_console.rs`) | angr i386-PIE PLT decode (DIV-9): `(kuna)` flag `analysis_i386_pie_plt` (decl + default-on init + `set_kuna_option`/env-bridge arm); `"i386_pie_plt"` in `KUNA_OPTION_NAMES`; `kuna_live_value` getter; `kuna_stages.cc` (`stages.toml`) settable row. New file `p0_knowledge/kuna_i386_pie_plt.rs` (env-var gate, no ElementId — loader-tier). The decode itself is in the **kuna-owned** `kuna-analysis::s1_loader::elf_plt::decode_i386` (kuna analog of Ghidra Java `ElfDefaultGotPltMarkup`, no C++ decompiler origin). `option i386_pie_plt`, default-on (ablation: 0 datatest assertions change). |
| `decompiler/cpp/coreaction.cc` (`ActionFinalStructure::apply`), `decompiler/cpp/architecture.{hh,cc}`, `decompiler/cpp/options.cc`, `decompiler/cpp/kuna_stages.cc` | angr break/continue recovery (port of Ghidra `BlockGraph::scopeBreak`): gated `graph.scopeBreak(-1,-1)` call between `finalizePrinting` and `markUnstructured`; flag `recover_loop_break` (decl + DIV-10 default-on init + reset + `set_kuna_option` parse + `build_arch_handle` copy); `KUNA_OPTION_NAMES` entry; settable + surface rows — all `(kuna)`, `option loopbreak_recovery` (ElementId 4092), DIV-10 default-on (clean ablation; `docs/divergences.md`) |
...
| `decompiler/cpp/flow.cc` | angr tee-O2 tail-jumps: include + `(kuna)`-gated block in `xrefControlFlow`'s `CPUI_BRANCH` arm — a direct `jmp` to another known function's entry (`kunaIsTailCallBranch`) is rewritten `BRANCH`→`CALL` + an artificial `RETURN` (the `truncateIndirectJump`/`setupCallindSpecs` halt-insert idiom) instead of flowing into the callee, with a `Funcdata::warning` (`tailcalljump: recovered tail call`) logging the introduced call; gated by `option tailcalljump`, default off |
| `decompiler/cpp/architecture.{hh,cc}`, `decompiler/cpp/options.cc`, `decompiler/cpp/kuna_stages.cc` | angr tee-O2 tail-jumps: flag `tail_call_jumps` (decl + default-off init + reset); `registerOption`/`set_kuna_option` arm + `KUNA_OPTION_NAMES`; settable + surface rows — all `(kuna)`, new file `kuna_tailcalljump.{hh,cc}` (ElementId 4101), default-off opt-in (ablation: 2 datatest assertions — `Long double #1/#2` — change if default-on) |
| `decompiler/cpp/funcdata.cc` (Rust: `infra/decompile_drive.rs`, `FlowEnvironment::queryCallNoReturn`), `decompiler/cpp/architecture.{hh,cc}` (`infra/architecture.rs`), `decompiler/cpp/options.cc` (`p0_knowledge/options.rs`), `decompiler/cpp/kuna_stages.cc` (`stages.toml`) | angr `test_tail_tail_bytes_ret_dup`: name-based no-return for **undefined externs**. `ArchFlowEnv::query_call_no_return` falls back, when the address-keyed check is false and the gate is on, to a name match (`kuna_noreturnextern::matches_noreturn_extern_name`, mirroring the analysis-tier `name_matches` over `ElfFunctionsThatDoNotReturn`); plants the existing `artificialHalt(noreturn)`. Flag `noreturn_extern_calls` (decl + default-false ctor + default-off reset + `on_off!` `set_kuna_option` arm); `KUNA_OPTION_NAMES` entry; `settableTable` row + `kuna-catalog.xml` provenance counts (14→15 angr `source_decompiler`, use_when/change_kind max 50→60). New module `kuna_noreturnextern.rs` (ElementId 4103). `option noreturn_extern`, **default-off opt-in** (no DIV): the name match overlaps `noreturn_known`'s for defined/imported symbols, so default-on would change PE/ELF `exit`-family handling under `noreturn_known off` (`verify_multiformat_passes::pe_exit_eliminates_dead_code_via_noreturn_list`); 675-datatest ablation 0/675, target 37% faster when on. |
...
`kuna_loopbreak_recovery.{hh,cc}` (port of Ghidra `BlockGraph::scopeBreak`, `option loopbreak_recovery`, ELEM 4092, DIV-10 default-on),
`kuna_gotoreduce.rs` (angr SAILR/Phoenix goto-reduction / ReturnDuplicator port, `option gotoreduce`, ELEM 4100, S8, default-off),
`kuna_tailcalljump.{hh,cc}` (angr tee-O2 tail-jump recovery, `option tailcalljump`, ELEM 4101, S2, default-off opt-in)
...
`sync_upstream.py`'s clean-apply guarantee **no longer holds** for the re-pinned
`tests/datatests/` files in the divergence table: a future sync touching them will need
`--3way` or manual conflict resolution. The sync remains clean for `specs/` and the
unmodified part of `tests/datatests/`.

## Build prerequisites

Only a **Rust toolchain** (`cargo`) is required — no C++, libbfd, bison, or flex
(those were only for the removed C++ tree). `make` builds everything under `decompiler/`.
...
- about added/deleted vendored files (rebuild and re-run `make test` after every sync).
...
sed -i 's|\([ab]\)/Ghidra/Features/Decompiler/src/decompile/datatests|\1/tests/datatests|g' /tmp/up.patch
