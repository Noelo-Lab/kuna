...
| `architecture.{hh,cc}` (`infra/architecture.rs`), `options.cc` (`p0_knowledge/options.rs`), `kuna_console.cc` (`kuna-console/src/kuna_console.rs`) | angr i386-PIE PLT decode (DIV-7): `(kuna)` flag `analysis_i386_pie_plt` (decl + default-on init + `set_kuna_option`/env-bridge arm); `"i386_pie_plt"` in `KUNA_OPTION_NAMES`; `kuna_live_value` getter; `kuna_stages.cc` (`stages.toml`) settable row. New file `p0_knowledge/kuna_i386_pie_plt.rs` (env-var gate, no ElementId — loader-tier). The decode itself is in the **kuna-owned** `kuna-analysis::s1_loader::elf_plt::decode_i386` (kuna analog of Ghidra Java `ElfDefaultGotPltMarkup`, no C++ decompiler origin). `option i386_pie_plt`, default-on (ablation: 0 datatest assertions change). |
