...
- **gh1537 + gh9230** (memsetrecover): the option fires but the recovery never emits `builtin_memset` — gh9230 leaves per-element stores, gh1537 (SIMD) emits only `buf[0]`. The `constseq`/`RuleMemsetCopy` reconstruction is incomplete on this tree.
...
- **namestyle #2/#6/#7**: `option namestyle ghidra` doesn't switch local naming (`v1` stays, never `iVar1`) and the call target prints `func_0x..` in both passes — the `name_style_angr` gate reaches some sites but not the local/call-target naming.
