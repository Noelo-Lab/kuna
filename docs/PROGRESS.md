...
- **gh1537 + gh9230** (memsetrecover): **confirmed deep.** `MemsetSequence::build_memset` (`kuna_memsetsequence.rs:269`) is a documented stub returning `None` — it needs `registerBuiltin(BUILTIN_MEMSET)`, the W6 `constructTypedPointer`/`inputTypeLocal` typed-pointer factory, and `removeCopyOps` (constseq COPY teardown). `RuleMemsetCopy` gates correctly but the body declines. Multi-seam (W4/W6) port; not attempted.
