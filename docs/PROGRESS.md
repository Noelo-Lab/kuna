...
## Session (2026-06-26) — name-matched extern no-return in ET_REL `.o` (option `noreturn_externmatch`, DIV-12)
...
**Mechanism**: new `option noreturn_externmatch` (S2 flow-follow, ElementId 4103). The
...
decompile). **Decision**: ship **default-ON** (DIV-12). One interaction noted: `option
...
dead code is gone. `docs/baseline-stages.json` 190 → 193 (+3). All gates green (catalog OK, datatest
PARITY OK, test-stages PARITY OK, rust-test green).
...
Landed (each its own commit; corpus-wide ones re-verified against the full 675):
- **sparcstructret gh6882 #1** (+1): the `is_sparc_struct_ret_trap` W4 flow-env override was an unwired stub — the predicate + S2 consumer were already ported. Wired it (post-call `unimp` falls through under `option sparcstructret on`).
- **o1return gh6882 #2 + gh6990 #1** (+2, +1 realign): the real gap behind the missing SPARC `o1`/CONCAT44 return wasn't trial collection — `ParamListStandardOut::fillinMap` was hardcoded to the legacy fallback, so the decoded `<join storage="general">` output ModelRule never ran. Wired the model path (`fspec.rs initialize`/`fillin_map_standard_out`); `buildReturnOutput` now emits `CONCAT44(o1,o0)`. gh6990 #2 realigned to the realtypes single-register form. **Corrected an earlier wrong call** that gh6990 was "subsumed" by heritage.
- **stackguard angr-StackGuard #1-4** (+4): `ActionStripStackGuard` detected the glibc canary but stopped at a seam — dropped in the real `remove_branch` + `remove_unreachable_blocks` surgery (unblocked precisely because fan-out's fix/heritage had landed `removeUnreachableBlocks`). The canary call/goto/label vanish and the deep path inlines `return 1`.
- **op::get_in Option contract** (robustness): `get_in(slot)` indexed `inrefs[slot]`, panicking on an out-of-range slot despite returning `Option`. `RuleExpandLoad` peeked `getIn(1)` before confirming a 2-input INT_ADD → crashed the ghangr-loweredswitch decompiles (LOSS-131). Made it honor the contract; the loweredswitch tests now run instead of erroring.
- **stackalias gh8500 #1/#2** (+2): NOT a `guard_stores` gap — a pre-existing **infinite heritage restart loop**: `bump_deadcode_delay` routed through a fresh `Override::new()` each call, so its "already installed" guard never fired → restart every pass → `MAX_REFLOW` → IR cleared → no body. Routed the bump through the persistent `localoverride` + re-applied per-space delays in `op_heritage`. A general deadcode-delay fix; the option is subsumed (test + baseline updated to the now-correct `return a0` both passes).
- **covermerge gh1276 #1/#2 + gh9218 #2** (+3): the deep S6 HighVariable cover/merge divergence, two coupled defects — (1) `set_varnode_properties` omitted the C++ `calcCover` tail, so `allocateCopyTrim` COPY phi-temps kept an EMPTY cover and an unrelated same-typed value merged into the loop-counter high; (2) unported `multipleInteraction`/`processMultiplier` (ActionMarkExplicit) left the borrow inlined. Ported both + `max_term_duplication`. This restored the faithful explicit-variable rendering, so **the 4 output-shape realignments from session-e (gh1282/gh7190/gh8017/gh8777) were reverted to their original calibrated strings** (the port made them correct — the workarounds were no longer right). gh9218 #1 realigned (realtypes); its malformed `while(if{return})` body is a separate, noted structuring bug.
- **e500 gh6904 #2/#3** (+2): the e500 r3:8/r3:4 input overlap is a faithful upstream throw provoked by the deliberately-partial e500 spec (both spec-extend and renamer-coalesce fixes ruled out as corpus-risky / out-of-scope). Instead made `IfcDecompile` degrade a recoverable LOSS-131 abort gracefully (print `Skipping`, return Ok), faithful to C++ `IfcProduceC::iterationCallback` — a per-function error no longer poisons the console. #1 (the full ~239-op e500 re-lift) stays out of scope.
- **loweredswitch ghangr-loweredswitch #1-7 + -mv #1-6** (+13): ported the INSTALL surgery (`Funcdata::kuna_install_lowered_switch`) — on the pre-SSA restart pass it severs the cascade head, installs a synthetic `BRANCHIND` + a hand-built trivial-model `JumpTable` with signed case labels, sweeps the orphaned spine, and lets the existing structurer/printer emit the `switch`.
- **loweredswitch switch-var threading** (follow-up): the synthetic BRANCHIND's switch-var read was nulled by heritage — the install built a 4-byte `EAX` read, but offset-0 is read 8-wide elsewhere (`RAX`), so `normalize_read_size` widened it to `SUBPIECE(<fresh 8-byte free read>,0)` whose source is undefined at the BRANCHIND and gets swept → `switch()`. Fixed by recording the switch-var storage on the synthetic `JumpTable` (`kuna_lowered_var`) and re-pointing the BRANCHIND input to the exact-size live reaching def in a post-heritage `kuna_repair_lowered_switch_inputs` (scoped to synthetic tables — the 675 corpus has none, so it's a no-op there). `switch()` → `switch(v3)`/`switch(v2)`.

Remaining (2, genuinely not cleanly fixable in scope): **KUNA-RESTARTS #1** — kuna recovers the jumptable fully in ONE pass (`is_partial=false`), so the upstream 1→>1 multistage restart never fires; recording it would require re-introducing incremental multistage flow recovery (massive, corpus-risky) or crippling kuna's recovery. **gh6904 #1** — the out-of-scope full e500 re-lift.

Process: the session-d worktree contamination (shared stash) did NOT recur — every agent this session was instructed to `git reset --hard <tip>` first and was barred from `git stash`. Integration was clean cherry-pick + full-gate throughout.
...
1. **Option flags never reached the pipeline.** A `Funcdata` reads its config off a lightweight **`ArchSeam`** (the *second* `Architecture`, `substrate/seams.rs`) built per-decompile by `Architecture::build_arch_handle`, which copied only a subset of the kuna flags. `present_lessequal` (compareform) was never copied, and the seam had no field at all for `fold_boolean_mask`/`fold_flag_compare`/`add_carry_chain`/`ov_less_simplify`/`recover_array_stride`/`memset_recover`/`model_stack_probe_loop`/`recover_lowered_switch`/`strip_stack_guard`. So an `option <name>` parse (and the DIV default) never took effect. Fix: added the seam fields + copied every flag in `build_arch_handle`, and switched each consuming Rule/Action gate to the live seam-read idiom `if !self.enabled && !data.get_arch().<flag> { return 0 }` (rules registered `enabled=false` in `universalaction.rs` so the flag drives both the default and the toggle; `enabled=true` stays the unit-test OR-override). stackprobeloop/loweredswitch were flipped `true`→`false` to read the flag.
2. **`option <kuna-name>` returned "Unknown option"** — the kuna options were never in the upstream `OptionDatabase`/`ElementId` registry. Added `Architecture::set_kuna_option` (reusing the existing `parse_*` helpers / `on_or_off`) and routed `KUNA_OPTION_NAMES` through it in `IfcOption`; the upstream `OptionDatabase` path is untouched (0 datatest churn).

Also: ported the `disassemble` console command (`IfcPrintdisasm` → SLEIGH `print_assembly` + `AssemblyEmit`, 12 SLEIGH decode tests now produce assembly), wired the 5 kuna infra console commands (`stage status`, `kassert` Option/Rename, `region tree/blocks/walk`, `quality` + `pipeline normalize`, `restarts` Part A), made `build_action` set the `decompile` current action (F1, faithful to upstream `resetDefaults`), and exposed the live `arraynotation` value in `stage catalog` (KUNA-CATALOG #2).

Gates: **`make test` 675/675 PARITY OK** (verified after every engine change); kuna-decomp + kuna-console unit tests green; **`make test-stages` 30 → 102 of 137 passing** (was effectively 30, with 31 files ERRORing on the option/disassemble seams).

The ~35 still-failing assertions are NOT the write-path seam: (a) the **realtypes↔xunknown corpus conflict** — `realtypes-fauxware` *requires* realtypes-on (no `xunknown`/`undefined`) while gh6930/gh9184/gh9218 assert the pre-realtypes `xunknown` form; gh6930's inferfuncentry actually works (name resolves), only the incidental type string differs; (b) **output-shape drift** — the rule fires correctly but structuring/CSE/literal-folding/naming differs from the calibrated strings (gh1282/gh1537/gh9230/gh7190/gh8913/gh8777/gh8017/namestyle); (c) **unported recovery logic** — loweredswitch install surgery, the v850/switchmod/stackalias/thumbfuncptr consumers, dynamichashmax budget (decompile path doesn't use it); (d) **caught pipeline panics** — gh6882/gh6990 (heritage `build_adt` asserts every non-root block has an idom; a guard balloons memory, needs the real `removeUnreachableBlocks`/`descend2Undef` port), gh6904 (partial e500 spec), gh1276 + ghangr-loweredswitch (rule logic panics on these inputs); (e) **pre-existing** — gh1243 (8051 body-drop), KUNA-RESTARTS #1 (multistage-jump trigger, W4 seam).

### Test realignment (same session) — realign calibrated strings to known-good output; note the rest as deep

Per the request to realign (not rewrite) the stage assertions where the divergence is a **known, correct** change, and to flag anything needing a code fix. Each `<stringmatch>` `name=` is left UNCHANGED (the baseline keys on names); only the pattern / `min`-`max` moved, and only where the targeted feature is demonstrably correct in the current output:

- **gh6930** (inferfuncentry) — the feature works (`evhttp_write_cb` resolves); realigned the incidental `*(xunknown8 *)`→`*(void *)` (realtypes pointer-to-unknown rendering). **PASS**.
- **gh7190 #1 / gh8777 #1 / gh8017 #1 / gh1282** — the kuna option transformation is correct (ovlesssimplify/flagcompare/stackprobeloop/booleanmask all toggle the right form); only CSE/explicit-variable/variable-numbering surface drifted (e.g. a cached `v2` inlined to `0 <= v1 + -7`; `v3`→`v2`; the folded subexpr used twice, so `max 1→2`). Valid C, realigned to the current form. **PASS**. (`tests/stages` 102→109/137; regressed 57→50.)

**Noted as needing a real code fix (NOT realigned — the output is wrong/incomplete, not just restyled):**
- **gh1537 + gh9230** (memsetrecover): **FIXED.** The "deep multi-seam" estimate was wrong — the sibling `StringSequence` (`RuleStringCopy`) already has the full machinery (`construct_typed_pointer`/`remove_copy_ops`/`collect_copy_ops`), and `BUILTIN_MEMSET` already existed (just never registered at boot). Ported `MemsetSequence` to reuse it: registered `BUILTIN_MEMSET`, added `StringSequence::build_for_fill`/`collect_fill_run`/`build_memset`/`transform_memset` (`pub(crate)`), factored the fill detection into `detect_fill_run`, and wired the live `RuleMemsetCopy::apply_op` (guards + `queryContainer` + build → detect → transform, mirroring `RuleStringCopy`). gh9230 → `builtin_memset(v1,0,100)`, gh1537 (SIMD) → the single memset; both pass (`tests/stages` 110 → 114/137). 675 corpus PARITY OK (no datatest has a constant-fill run).
- **gh8913 #2** (addcarrychain): the wide-add forms but `CONCAT11(0xd1,0x1a)` isn't collapsed to `0xd11a` (plus a stray `(uint2)` cast) — a post-fuse constant-fold/simplify is missing.
- **namestyle**: #2 **FIXED** — the `// eax` storage comment is an angr-only annotation, now gated on `name_style_angr` (printc.rs `emit_local_var_decls`; default-angr unchanged → 675-safe). #6/#7 remain **deep**: the ghidra `iVarN` naming (`Database::build_variable_name`) is **unreachable in the live pipeline** — there is NO non-test `impl DatabaseArch`, so `ActionNameVars` hardcodes `v{base}`. Wiring it needs the full `DatabaseArch` (types/translate/`type_name_base`) ported onto the seam (W4/W5/W6).
- **gh9184 #1**: kuna types the 256-bit YMM accumulator as a scalar `undefined32` vs upstream's `undefined1 [32]` array — a type-lane *inference* difference (width is correct), not a rendering one.
- **gh9218**: the merged input param is recovered (`a0`) but the control flow renders malformed (`while ( if … {return} …)`) — a structuring bug.
- (already deep from the main entry) gh6882/gh6990 heritage unreachable-block, gh6904 e500, gh1276 + ghangr-loweredswitch rule panics, gh1243 8051 body-drop, gh8471/gh8500/gh8817/gh9191 unported consumers, KUNA-RESTARTS #1.

## Session (2026-06-21c) — realtypes: real C types instead of `xunknownN` (DIV-6)

**Un-inferred values now render as real C types, not the fake `xunknownN`/`undefined<N>` placeholder.** Symptom: `kuna decompile fauxware authenticate` rendered `xunknown8 authenticate(xunknown8 a0,xunknown8 a1)` with `xunknown1 v2 [8]` / `xunknown4 v4` locals — Ghidra-style sized placeholders that aren't C types and read as noise. Fix (presentation-only, default-on, toggleable): a per-`Architecture` flag `realtypes` (set true in `resetDefaults`) read by the three printc declarator chokepoints (`declarator_parts` / `type_name_for_decl` / `push_cast_type`) via a `RealTypeCtx { enabled, long_is_8 }` resolved once per function in `doc_function_full`. A residual `TYPE_UNKNOWN` base is relabelled by size — 1→`char`, 2/4/8→`unsigned short`/`unsigned int`/`unsigned long` (or `unsigned long long` on LLP64; **unsigned** because the real sign is unknown — conservative, matches Ghidra's "TYPE_UNKNOWN is treated as an unsigned integer"), pointer-to-unknown (any depth)→`void *`, odd sizes (3/5/6/7/10/16…) keep `undefined<N>`. **Pure rendering**: only the printed NAME changes; the `Datatype`/metatype is untouched, so type propagation, merge, `ActionSetCasts`, and constant-literal formatting (`'a'` vs `0x61`) are all unchanged — which is exactly why it costs the parity gate nothing. fauxware now reads `unsigned long authenticate(unsigned long a0,unsigned long a1)` / `char v2 [8]` / `unsigned int v4`; genuinely-inferred types (`int4`, `uint1`, `float8`, `char *`) are left alone.

Registered as the 23rd kuna settable (`option realtypes off` restores upstream): `stages.toml` row (S9 `literal-format`, `change_kind=presentation-default`, `live_field=realtypes`), `KUNA_OPTION_NAMES`, console `kuna_live_value`; settable/live/comma/corpus counts bumped 22→23 / 19→20 / 21→22 / 131→132; catalog byte-compat fixture regenerated from the Rust emitter (C++ binary is gone) and `docs/assertions.md` regenerated. New testcase `tests/stages/realtypes-fauxware.xml` (the real `authenticate` bytes at 0x400664, no prototype → unknown types) asserts the real-C-type signature + `char[8]`/`unsigned int` + no `xunknown`/`undefined` survives (5/5 pass; `docs/baseline-stages.json` +5). Single-pass: the kuna option-toggle runtime write-path is an unimplemented seam on this tree, so off-behaviour is pinned by the printc unit test `realtypes_relabels_unknown_bases` instead. Two integration tests that pinned the old `xunknown4 promote_compare` signature were updated to `unsigned int promote_compare`.

Gates: **`make test` 675/675 PARITY OK** (corpus has no `xunknown`/`undefined` `<stringmatch>`, so 0 datatest churn); **`cargo test --workspace` 3754 passed / 0 failed**; `kuna catalog --check` OK. **`make test-stages` has a PRE-EXISTING failure on `main`** (135 regressed / 43 file-ERRORs from the unimplemented kuna-option write-path seam) — PROVEN unrelated to this change: clean-`main` and this branch have the identical 135-regression set (set-difference empty); my 5 new assertions pass in isolation. See `docs/divergences.md` DIV-6.
...
Verification: `kuna decompile fauxware main` → `puts(...)`, `read(...)`, `authenticate(...)`; spot-checked CET (`debug_symbol` → `fprintf`), PIE (`fdupes`/`posix_syscall` → no `sub_`). New gates — three real-binary loader tests (`loadimage_object.rs`: classic/CET-`.plt.sec`/stripped fixtures under `tests/fixtures/`), three synthetic per-arch decoder tests (`elf_plt.rs`), and a console e2e (`kuna-console/tests/verify_w11_elf_plt_names.rs`). Gate: **`make test` 675/675 PARITY OK** (datatests use the XML loader, never `ObjectLoadImage`, so this path is off the parity gate), `cargo --workspace --no-fail-fast` **3753 passed / 0 failed**. Report on the broader un-ported Ghidra application analyses (DWARF, demangling, string-literal typing, function-start discovery, library prototype seeding, …) in `docs/missing-analyses.md`.

## Session (2026-06-17c/18) — rust-port W10: 408 → 668/675; +...clone-gate audit, deindirect-output-type

**🎯 Local cross #2 +1 → 675/675 — M3 ACHIEVED: FULL DATATEST PARITY.** The deadcode-re-heritage diagnosis was misdirected; the lever was the STUBBED RulePullsubIndirect (ruleaction_1.rs:2019). On `EAX=SUB84(RAX_indcreate,0)` with upper RAX bytes unconsumed, C++ rewrites the RAX(8) killedbycall indirect-creation into a fresh EAX(4) one + replaces SUBPIECE descendants → collectOutputTrialVarnodes commits EAX(4) → `v1` not `(int4)v1`. Ported the deferred body (ruleaction.cc:963-1019, all seams pre-existed from RulePullsubMulti). Gate: **`[675,675]`**, regressed-set EMPTY, cargo --no-fail-fast 0-fail, PARITY OK. **THE RUST PORT NOW PASSES ALL 675 DATATEST ASSERTIONS, C++ ORACLE BYTE-UNTOUCHED.**

**Stack string #9 +1 → 674 (RulePieceStructure W4 mapentry seam — NOT RuleStoreVarnode/refinement).** The re-split `s..d1:1=DIL`→`v1[9]=a0` is RulePieceStructure (trace DEBUG 126), ported but declined two W4 mapentry seams: PieceNode::isLeaf (op.cc:831, returned leaf for ANY mapped vn) + determineDatatype/getStructuredType (used the vn's own 2-byte type not the containing char[40] Symbol). Fixed via the kuna mapentry containment query (findContainer keyed by SymbolId+offset+size): container_entry_key (varmap.rs) + vn_container_entry_key (funcdata.rs:1535) + piece_is_leaf/gather_pieces optional entry resolver + determine_datatype full body (ruleaction_6.rs:1199). Gate: `[675,674]`, regressed-set EMPTY, cargo 0-fail, PARITY OK. LAST ONE: Local cross #2 (LOSS-249 v2: deadcode-driven re-heritage RAX→EAX not firing — heritage.rs:3985 deadremoved path).

**UNIFIED ROOT CRACKED +3 → 673 (Long double #11 + Partial Merge #4/#5; the LOSS-248 root after 6 symptom-refinements).** The 6-wave heritage-disjoint/hole-fill/DCE chain was ALL SYMPTOM (proven byte-identical dual-engine). REAL root: kuna's ProtoStoreInternal keeps locked params OFF the local scope (C++ ProtoStoreSymbol::setInput fspec.cc:3174 maps them into the scope EntryMap at proto-attach) → rust findOverlap(s0x8,4)=None → no-symbol mapped|addrtied fallback kept addrforce on the dead width-4 hole-fill → addrForced INDIRECT survived+re-heritaged. Fix: link_proto_params at restructure_varnode head (funcdata_spacebase.rs:540, guarded has_store) so locked params are in-scope each restructure/sync → sync clears addrforce → DeadCode DCEs the INDIRECT+CONCAT22 → SEXT reads s0x8:2(i) → enabled the L4 inflateTest arm (coreaction_cleanup.rs:1269). Gate: `[675,673]`, regressed-set EMPTY, cargo 0-fail, PARITY OK. Remaining 2 are DISTINCT subsystems: Local cross #2 (return output-trial RAX/EAX sizing, funcdata_callsite.rs:403), Stack string #9 (post-heritage STORE re-introduction, RuleStoreVarnode re-fire).

**Gp Test #2 +1 → 670.** The HighVariable over-merge root was NOT merge.rs — it was the SAME register-param-usepoint issue as LOSS-244: add_param_symbol (varmap.rs) mapped EVERY recovered param with an INVALID usepoint → Scope::addMapInternal (database.rs:1943, empty uselimit) painted the register param `a0` ADDRTIED (fl=0x208000 mapped|addrtied, holdind=true) → mergeAddrTied grouped a0(i) with the &v1 PTRSUB COPY → `a0=v1;populate(a0)`. Fix (faithful, scoped to register storage so the join/stack-struct path is byte-unchanged): register-space param (no owning scope) mapped with restricted_usepoint (entry-1, fspec.cc:3170 ProtoStoreSymbol::setInput) not invalid → not addrtied; + link_proto_params baseaddr-1 + find_container_ignore_usepoint body-decl containment; then is_unmapped_unaliased flipped ON. Gained Gp Test #2 (gp constant folds → printf("Hello",a0), populate(v1)). Gate: `[675,670]`, regressed-set EMPTY (Gp #1 + all 48 switch + stack hold), cargo --no-fail-fast 0-fail, PARITY OK.

**Ccmp #1 +1 → 669.** RuleBooleanDedup (ruleaction_3.rs) had every guard ported but bailed at a STALE stub citation — the real BooleanMatch::evaluate (expression.rs:303, expression.cc:112) was ported but never wired. Transcribed the applyOp tail (ruleaction.cc:2851: the 4 isMatch partitions + De Morgan corners + (A||B)||(!A&&C) arm + newOp/opInsertBefore rewrite). `(b&&val==9)||(b&&val<9)` now factors to `b && (val<=9)` via RuleLessEqual → `if (*(char*)(a0+1)=='<' && a1<=9)`. Gained Ccmp test #1. Gate: `[675,669]`, regressed-set EMPTY, cargo --no-fail-fast 0-fail, PARITY OK.

**Partial Merge L1-L3 + Gp internal-storage substrates (enablers, +0) → 668.** Two regression-free substrates de-stubbing real upstream behavior: (EA) ActionDirectWrite drop has_store guard (coreaction.cc:1384) + bank_symbol_isolated via Symbol::isIsolated (merge.cc:196) + op-local-type interning (merge.cc:990) + isolated-symbol persistence across decompile — L4 inflateTest blocked on a spurious passmany int2 INDIRECT (heritage divergence, NOT the inflate machinery). (EB) ActionInternalStorage::apply de-stub (coreaction.cc:5192 setStoreUnmapped) + `<internal_storage>` cspec decode (was never parsed) + RuleStoreVarnode markNotMapped (ruleaction.cc:4360) — REFUTED the store-load-forwarding hypothesis for Gp #2 (it's MIPS gp being internal_storage); flip blocked on protect_switch_paths shield (coreaction.cc:2320) before wiring is_unmapped_unaliased. Both `[675,668]` regressed-EMPTY, cargo 0-fail, PARITY OK. (LOSS-248/247 next-loci sharpened.)

**AncestorRealistic killedbycall pop_fail (faithful de-stub, +0) → 668.** enter_node INDIRECT non-store branch (funcdata_varnode.rs:2965) now returns pop_fail for a killedbycall trial (funcdata_varnode.cc:2086, was stubbed) — a register input backtracing through an unknown_effect call-clobber INDIRECT is rejected, so Local cross #2's `call fretval` recovers with no args (input-half). Regression-free (`[675,668]`, cargo 0-fail, PARITY OK). Local cross #2 still BLOCKED on the output-half (RAX8-vs-EAX4 trial sizing / heritage refinement pass-ordering, LOSS-249). REMAINING 7 to M3 (all deep, sharpened next-loci in losses.md LOSS-244..249): Partial Merge #4/#5 (4-locus register coverage-merge), Gp Test #2 + Long double #11 + Stack string #9 (3 distinct pre-heritage fixpoints), Ccmp #1 (RuleConditionalMove float-NaN LOSS-074), Local cross #2 (output-half).
...
- **Tests:** `decompiler/unittests/` (204 C++ unit tests) + `tests/datatests/` (83 XML
