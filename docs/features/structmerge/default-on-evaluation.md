...
| (f) whole-corpus `decompile-all` before/after, ten binaries | 92 of 4,904 functions change; 91 are the record rename the option is for, 1 is a field respelling, 0 skeleton deltas, 0 declaration-count deltas, nothing unclassified. Those are this set of ten: on `e2fsck` -O2, outside it, 56 of 1,421 change with 2 respelled bodies and 1 declaration-count delta, every access still at the same address, and on `e2fsck` -O0 185 of 1,908 change with a class none of the ten hold: three variables LOSE their record for `void *` and `reconfigure_bool`'s two `char *` parameters become `unsigned long` (type_match 0.6667 -> 0.3333), against one function that gains a five-field record. The sweep's redo is the cause, not the union (`hunks.md`) |
