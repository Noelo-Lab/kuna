"""Interleaved min-of-N whole-binary decompile-all timing: structheadless off vs closed."""
...
ARMS = {"off": ["--option", "structheadless", "off"],
        "closed": ["--option", "structheadless", "closed"]}
NAMES = list(ARMS)
N = int(sys.argv[1]) if len(sys.argv) > 1 else 15
outf = sys.argv[2] if len(sys.argv) > 2 else "structheadless-speed.json"
...
for name in which:
    ...
    r["delta_min_pct"] = round((r["closed"]["min_ms"] - r["off"]["min_ms"]) / r["off"]["min_ms"] * 100, 2)
    r["delta_median_pct"] = round((r["closed"]["median_ms"] - r["off"]["median_ms"]) / r["off"]["median_ms"] * 100, 2)
    out[name] = r
    print(name, "off", r["off"]["min_ms"], "closed", r["closed"]["min_ms"],
