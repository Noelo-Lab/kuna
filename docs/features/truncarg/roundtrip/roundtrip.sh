...
#
# Builds rt.c with gcc -O2, decompiles its six narrowed-argument callers with the
...
cp "$HERE/rt.c" "$W/rt.c"
cd "$W"
gcc -O2 -fno-inline -o rt rt.c
./rt > bin.out
: > decomp.c
for fn in f k a2 neg ci h; do "$K" decompile rt "$fn" >> decomp.c; done
{
