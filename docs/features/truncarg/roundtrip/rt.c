...
unsigned char acc8; int accs;
NI void r1(signed char c) { acc8 = c; sinki(c); }
NI void r6(unsigned char c) { acc8 = c; sinki(c); }
NI void r7(signed char c) { accs = c > 10; sinki(c); }
...
int main(void) {
  for (unsigned i = 0; i < sizeof vals / sizeof vals[0]; i++) {
    ...
    r1((signed char)v); r6((unsigned char)v); r7((signed char)v);
  }
