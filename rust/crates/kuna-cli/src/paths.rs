...
//! `kuna/paths.py`.
//!
//! The `kuna` binary normally lives at `rust/target/<profile>/kuna`, so its
...
/// `<root>/rust/target/<profile>/kuna`, so the root is four parents up.  Falls
/// back to the current directory if that walk fails.
pub fn repo_root() -> PathBuf {
    ...
    if let Some(dir) = exe_dir() {
        // <root>/rust/target/<profile>/<exe>  ->  pop profile, target, rust.
...
pub fn datatests_dir() -> PathBuf {
    repo_root().join("decompiler").join("datatests")
}
...
/// `rust/target/<KUNA_RUST_PROFILE or release>/<name>` under the repo root.
pub fn binary(name: &str, env_var: &str) -> PathBuf {
    ...
    repo_root().join("rust").join("target").join(profile).join(name)
}
