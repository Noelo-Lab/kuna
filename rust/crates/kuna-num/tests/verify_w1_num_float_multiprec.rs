...
// ---------------------------------------------------------------------------
// Round 2 (verifying the F1 repair, commit 9d5d354).  Oracle values below
// were regenerated by the round-2 verifier with a FRESH standalone harness
// linking the vendored decompiler/cpp/float.cc (g++ -O2, x86-64 glibc).
// ---------------------------------------------------------------------------

/// The full carry-to-normal cell FAMILY around the repaired masked shift
/// (float.rs:522/:607, C++ float.cc:343/:416).  The family is wider than the
/// single F1 cell: ANY host whose significand has its top 23 fraction bits
/// all ones and that rounds up in the fmt4 denormal path carries out of bit
/// 63, takes the `exp == 1` fixup, and must come back as the (signed) zero
/// encoding via the count-masked shift -- the x86-64 C++ oracle returns 0
/// for every such cell.  Neighbouring legitimate cells (exact min normal,
/// in-denormal carry exp -1 -> 0, min-denormal cluster, below-min-zero) pin
/// the boundary of the repair on both sides.
#[test]
fn verify_r2_carry_to_normal_family_oracle() {
    let fmt4 = FloatFormat::new(4);
    let fmt8 = FloatFormat::new(8);
    let cells: [(u64, u64); 14] = [
        // -- carry-to-normal (UB fixup, exp == 1, masked shift) --
        (0x380f_ffff_ffff_ffff, 0x0000_0000), // (1-2^-53)*2^-126
        (0xb80f_ffff_ffff_ffff, 0x8000_0000), // negative of the above
        (0x380f_ffff_ffff_fffe, 0x0000_0000),
        (0x380f_ffff_ffff_fc00, 0x0000_0000),
        (0x380f_ffff_ffff_fbff, 0x0000_0000), // top-23-ones + round-up: still carries
        (0x380f_ffff_ffff_f800, 0x0000_0000),
        // -- legitimate neighbours (no UB) --
        (0x3810_0000_0000_0000, 0x0080_0000), // 2^-126 exact: smallest normal
        (0x37ff_ffff_ffff_ffff, 0x0040_0000), // just below 2^-127: carry stays denormal (exp -1 -> 0)
        (0x37f0_0000_0000_0000, 0x0020_0000), // 2^-128 exact
        (0x36a0_0000_0000_0000, 0x0000_0001), // 2^-149 exact: min denormal
        (0x3690_0000_0000_0000, 0x0000_0000), // 2^-150: tie with zero -> even
        (0x3690_0000_0000_0001, 0x0000_0001), // just above 2^-150
        (0xb690_0000_0000_0001, 0x8000_0001), // negative of the above
        (0x368f_ffff_ffff_ffff, 0x0000_0000), // just below 2^-150: early exp < -frac_size zero
    ];
    for (bits, expect) in cells {
        assert_eq!(
            fmt4.get_encoding(f64::from_bits(bits)),
...
/// fmt8 deep-denormal cells: the LEGITIMATE large shift counts of the
/// repaired line (`-exp` up to 51 for f64 denormals).  `wrapping_shr` must
/// behave as a plain shift for every in-range count; the C++ oracle
/// round-trips each denormal double encoding exactly through getEncoding.
#[test]
fn verify_r2_fmt8_deep_denormal_shift_counts() {
    let fmt8 = FloatFormat::new(8);
    let cells: [u64; 6] = [
        0x0000_0000_0000_0001, // 2^-1074: shift count 51
        0x0000_0000_0000_0003, // shift count 50
        0x0000_0000_0000_0007,
        0x000f_ffff_ffff_ffff, // largest denormal double: shift count 0
        0x0010_0000_0000_0000, // smallest normal (normal path)
        0x8000_0000_0000_0001, // negative min denormal
    ];
    for bits in cells {
        assert_eq!(
            fmt8.get_encoding(f64::from_bits(bits)),
            bits,
            "get_encoding(fmt8, {bits:#018x}) must round-trip"
        );
    }
}

/// 200,000-cell differential sweep over the fmt4 denormal/boundary window
/// (exponent fields 0x365..=0x382, random mantissas and signs), replayed
/// bit-for-bit from the C++ oracle harness.  The FNV-1a-64 hash chains
/// (bits, getEncoding(fmt4), convertEncoding(fmt4<-fmt8)) per cell; the
/// constant below is the hash the REAL vendored float.cc produced.  Any
/// single-cell divergence (including a debug-only panic) breaks the hash.
#[test]
fn verify_r2_denormal_window_sweep_hash_matches_cpp_oracle() {
    let fmt4 = FloatFormat::new(4);
    let fmt8 = FloatFormat::new(8);
    let mut x: u64 = 0x6b75_6e61_5f77_3153; // seed (shared with the C++ driver)
    ...
    for _ in 0..200_000 {
        x = x
            .wrapping_mul(6364136223846793005)
            .wrapping_add(1442695040888963407);
        let e = 0x365u64 + ((x >> 52) % 0x1e); // exponent field 0x365..=0x382
        let m = x & 0x000f_ffff_ffff_ffff;
        let s = (x >> 21) & 1;
        let bits = (s << 63) | (e << 52) | m;
        let enc = fmt4.get_encoding(f64::from_bits(bits));
        let cnv = fmt4.convert_encoding(bits, &fmt8);
        h = (h ^ bits).wrapping_mul(FNV);
        h = (h ^ enc).wrapping_mul(FNV);
        h = (h ^ cnv).wrapping_mul(FNV);
    }
    assert_eq!(h, 0x28db_e215_fe70_2925, "sweep hash vs C++ float.cc oracle");
}
