...
picks, which is cheap and deterministic.
    ...
    python -m scripts.pipeline.sweep                          # full sweep -> opportunities.json + matrix.md
    python -m scripts.pipeline.sweep --arch x86_64 --jobs 6   # x86_64/i386 subset, 6 concurrent
    python -m scripts.pipeline.sweep --limit 20 --print       # quick look, no write
"""
...
def to_matrix_md(ranked, ref_version):
    ...
    lines.append("")
    lines.append("**Generated** by `python -m scripts.pipeline.sweep` -- do not edit by hand.")
    lines.append("")
...
def main(argv=None):
    p = argparse.ArgumentParser(prog="python -m scripts.pipeline.sweep",
