...
check-deps:
	...
	@echo 'int main(){return 0;}' | g++ -x c++ - -lbfd -o /dev/null 2>/dev/null \
	  || { echo "ERROR: libbfd missing -> sudo apt-get install -y binutils-dev"; exit 1; }
...
binaries: check-deps touch-generated
	$(MAKE) -C $(CPPDIR) -j$(NJOBS) sleigh_opt
	$(MAKE) -C $(CPPDIR) -j$(NJOBS) decomp_dbg
	$(MAKE) -C $(CPPDIR) -j$(NJOBS) decomp_opt
	$(MAKE) -C $(CPPDIR) -j$(NJOBS) decomp_test_dbg
