...
const uint4 UserPcodeOp::BUILTIN_MEMSET = 0x10000006;	// (kuna) GH-9230 constant-fill recovery
...

{
  ...
  switch(i) {
    ...
    case UserPcodeOp::BUILTIN_MEMSET:		// (kuna) GH-9230 void *memset(void *dst,int val,int len)
    {
      int4 ptrSize = glb->types->getSizeOfPointer();
      int4 wordSize = glb->getDefaultDataSpace()->getWordSize();
      Datatype *vType = glb->types->getTypeVoid();
