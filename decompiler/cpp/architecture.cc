...

{
  ...
  infer_pointers = true;
  infer_funcentry = true;	// (kuna) DIV-2 default-on: function entries at single-bit bases are inferred (GH-6930)
  return_single = false;	// (kuna) default: upstream behavior (join register pairs)
  memset_recover = true;	// (kuna) DIV-2 default-on: constant-fill runs recover as builtin_memset (GH-9230/1537)
  v850_indirect_branch = false;	// (kuna) default: upstream behavior (V850 jmp [reg] stays CALLIND) (GH-8817)
  sparc_struct_return = false;	// (kuna) default: upstream byte-identical (SPARC post-call unimp stays a terminator) (GH-6882)
  ...
  input_varnode_adjust = true;	// (kuna) DIV-3 default-on: absorb overlapping input varnodes (GH-9218)
  dynamic_hash_maxdup_high = true;	// (kuna) DIV-3 default-on: collision budget 16 (GH-8467)
  fold_flag_compare = true;	// (kuna) DIV-3 default-on: fold flag-compare idioms (GH-1276/8777)
  ...
  recover_lowered_switch = false;	// (kuna) default: upstream byte-identical (lowered comparison-cascade switch recovery off)
