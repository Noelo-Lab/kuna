...
static const KunaSubStage substageTable[] = {
  ...
  { "loop-refinement", kstage_s7,
    ...
    "LATENT in Ghidra/Reko (angr-only options); kuna `region tree/blocks/walk` (KunaRegionIdentifier) = observable half" },
...
static const KunaSurfaceEntry surfaceTable[] = {
  ...
  { "option stackguard", kstage_s7, "edge-virtualization", "(kuna angr-port) strip the -fstack-protector canary epilogue so the shared-return goto is eliminated" },
  ...
  { "region tree", kstage_s7, "loop-refinement", "(kuna angr-port) analysis-only nested region tree over bblocks (KunaRegionIdentifier; angr GraphRegion.dbg_print)" },
  { "region blocks", kstage_s7, "loop-refinement", "(kuna angr-port) flat regions_by_block_addrs dump per identified region" },
  { "region walk", kstage_s7, "loop-refinement", "(kuna angr-port) recursive region-order walk over every leaf basic block" },
...
static const KunaSettable settableTable[] = {
  { "compareform", "canonical|original", "original", false,
    ...
    "ghidra-upstream", "Ghidra upstream comparison rendering (GH-558 prototype)", "presentation-default" },
  { "arraynotation", "on|off", "on", false,
    ...
    "ghidra-upstream", "Ghidra upstream pointer-arithmetic rendering", "presentation-default" },
  { "thumbfuncptr", "on|off", "on", false,
    ...
    "ghidra-upstream", "Ghidra GH-8471 (ARM/Thumb function-pointer recovery)", "correctness-fix" },
  { "inferfuncentry", "on|off", "on", false,
    ...
    "ghidra-upstream", "Ghidra GH-6930 (function-entry constant inference)", "correctness-fix" },
  { "booleanmask", "on|off", "on", false,
    ...
    "ghidra-upstream", "Ghidra GH-1282 (boolean sign-extension-mask idiom)", "correctness-fix" },
  { "ovlesssimplify", "on|off", "on", false,
    ...
    "ghidra-upstream", "Ghidra GH-7190 (S/OV-flag compare idiom)", "correctness-fix" },
  { "addcarrychain", "on|off", "on", false,
    ...
    "ghidra-upstream", "Ghidra GH-8913 (8-bit ADC carry-chain recovery)", "correctness-fix" },
  { "memsetrecover", "on|off", "on", false,
    ...
    "ghidra-upstream", "Ghidra GH-9230/1537 (constant-fill store run -> builtin_memset)", "correctness-fix" },
  { "returnpair", "pair|single", "pair", true,
    ...
    "ghidra-upstream", "Ghidra GH-6990 (wide return-register joining)", "opt-in-tool" },
  { "v850indirectbranch", "on|off", "off", true,
    ...
    "ghidra-upstream", "Ghidra GH-8817 (V850 jmp[reg] flow classification)", "opt-in-tool" },
  { "flagcompare", "on|off", "on", false,
    ...
    "ghidra-upstream", "Ghidra GH-1276/8777 (flag-modelled comparison idioms)", "correctness-fix" },
  { "switchmodbound", "on|off", "off", true,
    ...
    "ghidra-upstream", "Ghidra GH-9191 (modulo/and-mask jumptable bound)", "opt-in-tool" },
  { "stackprobeloop", "on|off", "on", false,
    ...
    "ghidra-upstream", "Ghidra GH-8017/6858 (stack-probe loop SP normalization)", "correctness-fix" },
  { "dynamichashmax", "on|off", "on", false,
    ...
    "ghidra-upstream", "Ghidra GH-8467 (DynamicHash same-address collision budget)", "correctness-fix" },
  { "stackalias", "on|off", "off", true,
    ...
    "ghidra-upstream", "Ghidra GH-8500 (stack-pointer-alias store liveness)", "opt-in-tool" },
  { "arraystride", "on|off", "on", false,
    ...
    "ghidra-upstream", "Ghidra GH-8724 (strided-array index recovery)", "structure-recovery" },
  { "sparcstructret", "on|off", "off", true,
    ...
    "ghidra-upstream", "Ghidra GH-6882 (SPARC struct-return unimp fallthrough)", "opt-in-tool" },
  { "condexeplace", "on|off", "on", false,
    ...
    "ghidra-upstream", "Ghidra GH-9203 (ActionConditionalConst loop-block COPY placement)", "correctness-fix" },
  { "inputvarnodeadjust", "on|off", "on", false,
    ...
    "ghidra-upstream", "Ghidra GH-9218 (overlapping input-varnode absorb)", "correctness-fix" },
  { "loweredswitch", "on|off", "on", false,
    ...
    "angr", "angr LoweredSwitchSimplifier (angr/analyses/decompiler/optimization_passes); test_decompiler.py switch-lowering cases", "structure-recovery" },
  { "stackguard", "on|off", "off", true,
    kstage_s7, "edge-virtualization", kstrength_hard, kstage_s7, "angr-StackCanarySimplifier",
    "Strip the glibc -fstack-protector canary epilogue (the `if (canary != *(fs:0x28)) __stack_chk_fail()` check + its no-return call block), collapsing the shared single-return point so each path returns directly (no goto/label).",
    "Set on to remove compiler-inserted stack-protector boilerplate and the shared-return goto it forces; off (default) is upstream byte-identical. DESTRUCTIVE as a global default: it deletes the real canary-check instructions (and may match an unusual non-glibc `ptr+0x28` compare guarding a call).",
    "option stackguard on",
    "angr", "test_ite_region_converter_missing_break_statement; angr StackCanarySimplifier (angr/analyses/decompiler/optimization_passes/stack_canary_simplifier.py); authenticate", "opt-in-tool" },
  { "namestyle", "angr|ghidra", "angr", false,
    ...
    "angr", "angr decompiler default naming scheme (vN / aN / sub_<addr> / dat_<addr> / label_<addr>)", "presentation-default" }
};
