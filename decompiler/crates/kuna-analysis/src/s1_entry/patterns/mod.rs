...
//! [`FuncStartPatternPass`](crate::s1_entry::FuncStartPatternPass), gated
//! default-**off** by `--option funcstart_patterns on`). The existing oracle 5
//! (`s1_entry::prologue_pattern_starts`) vendors a hand-written **minimal** set of
...
//! the same wall `s1_entry` documents for the `after="defined"`/`validcode` rules):
//!
...
//!   `s1_entry`'s module docs record). We keep the `<funcstart/>` post action of a
...
//! ## Provenance
//!
//! The XMLs under `s1_entry/patterns/*.xml` are vendored **verbatim** from
