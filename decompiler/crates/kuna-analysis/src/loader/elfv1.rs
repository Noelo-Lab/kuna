...
impl Descriptors {
    ...
    pub fn read(file: &object::File<'_>) -> Self {
        Self::read_candidates(file, false)
    }

    /// Scan aligned .opd words conservatively for import ambiguity, without supplying loader seeds.
    pub(super) fn read_for_imports(file: &object::File<'_>) -> Self {
        Self::read_candidates(file, true)
    }

    fn read_candidates(file: &object::File<'_>, scan: bool) -> Self {
        ...
        addresses.extend(file.symbols().chain(file.dynamic_symbols())
            ...
            .map(|s| s.address()));
        if scan {
            let first = ((8 - opd.address() % 8) % 8) as usize;
            addresses.extend((first..bytes.len()).step_by(8).map(|n| opd.address() + n as u64));
        }
        ...
        for address in addresses {
            ...
            let Some(record) = bytes.get(offset..) else { continue; };
            if !scan && record.len() < 24 { continue; }
            let word = |start| {
                let b: [u8;8] = record.get(start..start+8)?.try_into().unwrap();
            ...
            let Some(entry) = word(0) else {
                if scan { return Self::default(); }
                continue;
            };
            let toc = word(8);
