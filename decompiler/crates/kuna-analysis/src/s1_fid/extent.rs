...
//! [`crate::s1_noreturn_disc`] already uses for its call-site / fall-through
//! reasoning.
//!
...
//! is mis-clipped. This is the same approximation `s1_noreturn_disc` accepts and
...
/// [`crate::s1_fid::hash::FidHasher`] (PR4 turns each `&Insn` + its SLEIGH mask
/// into an [`crate::s1_fid::hash::InsnFingerprint`]).
///
/// The hasher itself rejects extents below
/// [`SHORT_HASH_CODE_UNIT_LENGTH`](crate::s1_fid::hash::SHORT_HASH_CODE_UNIT_LENGTH);
