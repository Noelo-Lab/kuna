...
//!   `kuna_pdb_path` env var, the `s1_fid` `kuna_fid_db` precedent) + the
...
//!
//! [`PdbPass`] is the `.pdb`-consuming `AnalysisPass`, modeled on [`crate::s1_dwarf`]
//! (the type/name producer) + the [`crate::s1_fid`] external-artifact precedent
...
impl AnalysisPass for PdbPass {
    ...
    fn run(&self, ctx: &AnalysisCtx) -> AnalysisOutput {
        ...
        // 2. Tier-1 locate: the `kuna_pdb_path` env var (the s1_fid precedent).
        //    Unset/empty ⇒ inert.
...
/// `None` on a non-PE / unparsable image. Self-contained (the `s1_rtti`
