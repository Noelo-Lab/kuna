...
//! [`crate::s1_pdb::codeview`]), never the debug info itself. So PDB recovery is
...
//! `s1_fid` `kuna_fid_db` env-var precedent.
//!
//! ## Tier-1 (the MVP, this PR)
//!
//! The `.pdb` path is read from the **`kuna_pdb_path` environment variable** — the
//! exact `s1_fid::FID_DB_ENV` (`kuna_fid_db`) precedent. This is the simplest
...
use crate::s1_pdb::codeview::CodeViewInfo;

/// The environment variable naming the external `.pdb` file to apply. Mirrors the
/// `s1_fid` `kuna_fid_db` env gate exactly (the FID-database external-artifact
