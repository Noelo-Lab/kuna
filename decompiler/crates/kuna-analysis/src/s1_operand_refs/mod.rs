...
//! elsewhere in kuna (subroutine refs → `s1_entry`; jump tables → the engine's
//! `JumpTable::recoverAddresses` switch recovery / `s1_addrtable`; PLT/GOT names →
//! `s1_loader::elf_plt`). See the "covered-elsewhere" map below.
//!
...
//!    are already named by [`crate::s1_loader::elf_plt`], so a scalar pointing at
...
//!   is already planted as a `char[N]` by the always-on [`crate::s1_strings`] pass,
...
//!   ([`crate::s1_protos`] / S5 usage inference). So this pass only adds output for
//!   the *residual* case: a short (< 5 char) or otherwise `s1_strings`-missed
...
//!   false-positive shape that keeps [`crate::s1_addrtable`] off by default.
//!
...
//! |---|---|
//! | subroutine reference → create function | [`crate::s1_entry`] (entry discovery) |
//! | jump/address-table reference | engine `JumpTable::recoverAddresses` (S2) + [`crate::s1_addrtable`] |
//! | string reference | [`crate::s1_strings`] + the printer literal route |
//! | `.plt`/`.got` reference (corrective) | [`crate::s1_loader::elf_plt`] names them directly |
//!
...
/// [`crate::s1_strings`] pass uses 5; this pass is the value-add for the residual
...
/// Reuses the [`crate::s1_strings`] printable-char recognizer so a planted symbol
/// is shaped identically. Used to emit a [`StringFact`] for the residual short /
/// `s1_strings`-missed strings this pass is the value-add for.
