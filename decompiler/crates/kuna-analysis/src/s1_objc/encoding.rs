...
//! name-level-opaque posture as `s1_dwarf`'s MVP). Protocol qualifiers
...
impl<'a> Parser<'a> {
    ...
    fn next_type(
        ...
        let ty = match c {
            ...
            // placeholder (the s1_dwarf recursion-cap posture). Reconstructing the
...
/// `s1_dwarf` `get_type_struct` posture. Falls back to a plain `void*` if the
