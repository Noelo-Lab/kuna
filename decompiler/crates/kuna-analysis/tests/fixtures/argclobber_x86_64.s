...
# quotient. `target` reads rdi and rsi and nothing else, so its recovered
# prototype takes two parameters -- exactly the two the call is left with -- and
# says rdx is free.
#
...
target:  push %rbx;  mov %rdi,%rbx;  add %rsi,%rbx;  call helper;  mov %rbx,%rax;  pop %rbx;  ret
helper:  ret
