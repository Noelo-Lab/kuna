/* (kuna castsign) Values an -O0 build keeps in a stack slot and compares
 * signed, read with strtoul so a test can pass the edges of the range.  The
 * first five do unsigned arithmetic on the value (a decrement, an increment, a
 * difference, a self-update) whose result the program compares signed.  That
 * arithmetic wraps at 2^63 (2^31 for dec_neg32) and would overflow if the value
 * were declared signed, so `castsign` must leave those declarations unsigned.
 * The last two only compare, assign and mask the value, so `castsign` declares
 * it signed.  The round-trip test in kuna-cli/tests/decompile_all_cli.rs
 * compiles the functions as kuna prints them with gcc and clang at -O0 and -O2,
