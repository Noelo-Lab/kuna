...
| `rust_variant_x86_64` | tiny `#![no_std]` rustc **non-PIE** (`-C relocation-model=static`) x86-64, **not stripped** (source `rust_variant_x86_64.rs`) | the rustc **tagged two-variant return** (`option rustadt`, P5; e2e `kuna-console/tests/verify_rustadt_variant.rs`). `branchy`@`0x201270` is `fn(u32,u32) -> Result<u32,u32>` written with three explicit `return`s, so `-C opt-level=2` emits three `RETURN`s whose tag is a LITERAL on each (`xor %eax,%eax` twice, `mov $0x1,%eax` once) rather than the branchless `setCC` of `rust_scalarpair_x86_64` — that literal-per-path form is exactly what the rule requires. `samevariant`@`0x2012a0` is the NEGATIVE: the same signature written without a second variant, which rustc compiles branchlessly to a SINGLE return, so there are not two tags to group and nothing may be synthesized. `.comment` carries the `rustc version` record, so `option rustadt auto` fires here without `always`. `branchy`'s bytes are the first `<bytechunk>` in `tests/stages/kuna-rustadt.xml` verbatim |
...
`rust_variant_x86_64` (2168 bytes, source vendored alongside as
`rust_variant_x86_64.rs`): same rustc and the same
`-C relocation-model=static` reason as `rust_scalarpair_x86_64` — fixed VMAs so
the stage testcase can embed the bytes. The shape the fixture exists for is
`branchy`'s: THREE explicit `return`s in the source, which is what makes rustc
materialize the discriminant as a literal per path instead of the branchless
`setCC` it prefers when the whole function is one expression. `option rustadt`
groups return paths by that literal, so the branchless form is out of its scope
and this fixture is the only one that exercises the rule at all.
