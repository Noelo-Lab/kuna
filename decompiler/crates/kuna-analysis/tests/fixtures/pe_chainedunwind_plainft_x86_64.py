#!/usr/bin/env python3
"""Generate `pe_chainedunwind_plainft_x86_64.exe` — a minimal PE32+ whose `.pdata` splits
...
pattern as `crtmain_x86_64.py`).  Regenerate with:
    ...
    python3 pe_chainedunwind_plainft_x86_64.py
...
def build_text():
    ...
    # ---- primary [0x1000, 0x1020): ends in a *fall-through* into CHUNK -------
    p = PRIMARY
    p = put(p, bytes([0x48, 0x83, 0xEC, 0x28]))          # sub  rsp,0x28
    p = put(p, bytes([0x8B, 0xC1]))                      # mov  eax,ecx
    p = put(p, bytes([0x83, 0xC0, 0x01]))                # add  eax,1
    p = put(p, bytes([0x85, 0xC0]))                      # test eax,eax
    disp = COLD - (p + 6)
    p = put(p, b"\x0F\x84" + struct.pack("<i", disp))    # jz   COLD
    for imm in (2, 3, 4, 5, 6):
        p = put(p, bytes([0x83, 0xC0, imm]))             # add  eax,imm
    assert p == CHUNK, hex(p)                            # falls through into CHUNK
...
if __name__ == "__main__":
    out = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                       "pe_chainedunwind_plainft_x86_64.exe")
