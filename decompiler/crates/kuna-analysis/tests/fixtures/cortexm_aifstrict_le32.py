#!/usr/bin/env python3
"""Generate `cortexm_aifstrict_le32` — the GH-299 witness for `aifstrict`: a
stripped bare-metal ARM Cortex-M image in which AIF's byte-granular gap cursor
plants a function entry one halfword INSIDE a literal pool and, in doing so,
consumes the real function that pool precedes.
...
hand-encoded. Regenerate with:
    ...
    python3 cortexm_aifstrict_le32.py

Layout:

  PT_LOAD [0x08000000, 0x08000168)  PF_R|PF_X
  ...
  .text  @0x08000000  SHF_ALLOC|SHF_EXECINSTR
      ...
      0x08000148 POOL1   .word 0x20001000        <- the hole
      0x0800014c B       movs r1,#6 ; movs r2,#7 ; movs r3,#8 ; bx lr
      0x08000154 C       push {r7,lr} ; ldr r0,[pc,#4] ; movs r1,#2 ; pop {r7,pc}
      0x0800015c POOL2   .word 0x20001000        <- the second hole
      0x08000160 D       movs r2,#9 ; adds r2,#1 ; adds r2,#1 ; bx lr

`H1..H20` are two gates at once. AIF refuses to run at all on a program with fewer
than `MINIMUM_FUNCTION_COUNT` (20) discovered functions, and it accepts a candidate
only when its two-mnemonic prologue fingerprint is shared by at least
`FINGERPRINT_THRESHOLD` (4) of them; twenty helpers whose first two instructions are
`movs ; movs` over four bytes clear both. `reset` calls every helper plus `A` and
`C` directly, so the recursive-descent walk reaches them and stops at each
`pop {r7,pc}`; `B` and `D` are called by nobody.

  * **THE DEFECT** — `POOL1` is data, so it is an undefined hole that runs straight
    through `B`'s body (the next decoded instruction is `C`). The cursor rejects
    `POOL1`, then slides ONE BYTE at a time and probes `POOL1 + 2`. The pool word is
    little-endian, so the halfword there is `0x2000` — `movs r0,#0` — and the next
    halfword is `B`'s own `movs r1,#6`: a synthetic `movs ; movs` pair identical to
    the helpers' fingerprint. AIF accepts `0x0800014a`, and because an accept
    advances the cursor past the accepted body, `B`'s real entry at `0x0800014c` is
    never probed at all. The false entry does not accompany the true one, it
    REPLACES it, and the body it names carries a dead leading `movs r0,#0` the
    decompiler then folds through the whole function.

  * **THE FIX** — `0x0800014a` is 2-mod-4 and is not the first byte of its hole, so
    `aifstrict`'s aligned slide never probes it. The cursor's next candidate after
    `POOL1` is `POOL1 + 4`, which is `B` itself.

  * **THE CONTROL** — `C` / `POOL2` / `D` is the same shape with one difference:
    `D` opens `movs ; adds`, a fingerprint no discovered function shares. The
    aligned probe at `POOL2 + 4` is therefore REJECTED. `aifstrict` only declines to
    probe addresses that cannot be instruction boundaries; it never lowers the
    acceptance bar, and this shape is what proves the fingerprint gate still ran.
"""
import struct, os
...
def build_text():
    ...
    # reset: call every discovered function so the walk finds them all. `H[0]` is
    # called twice purely so the 23 four-byte `BL`s land the closing `pop` on
    # `H[0] - 2`, keeping the helper block (and everything after it) word-aligned.
    t += hw(PUSH_R7_LR)
    for target in [*H, H[0], A, C]:
        t += bl(at(), target)
    t += hw(POP_R7_PC)
    ...
    # A / POOL1 / B — THE DEFECT: the phantom the byte-slide accepts at POOL1+2,
    # and the real function it consumes.
    assert at() == A, hex(at())
    ...
    # C / POOL2 / D — THE CONTROL (`D` opens `movs ; adds`, an unshared fingerprint).
    assert at() == C, hex(at())
...
if __name__ == '__main__':
    out = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'cortexm_aifstrict_le32')
