#!/usr/bin/env python3
"""Generate `pe_chainedunwind_x86_64.exe` — a minimal PE32+ whose `.pdata` splits
...
pattern as `crtmain_x86_64.py`).  Regenerate with:
    ...
    python3 pe_chainedunwind_x86_64.py
...
def build_text():
    ...
    # ---- primary [0x1000, 0x1020): the LAST instruction is a forward JZ whose
    # FALL-THROUGH is CHUNK (exactly GH-403's `14000270c JZ 0x140002d6b`). ------
    p = PRIMARY
    ...
    disp = COLD - (p + 6)
    p = put(p, b"\x0F\x84" + struct.pack("<i", disp))    # jz   COLD (fall-through = CHUNK)
    assert p == CHUNK, hex(p)
