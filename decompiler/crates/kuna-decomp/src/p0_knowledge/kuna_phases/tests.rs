...
fn settable_count_is_193() {
    ...
    // +1 for `nulterminator` (P6 stack-array terminator absorption, DIV-173).
...
fn tier_counts_are_62_core_72_transform_59_analysis() {
    ...
    // completion, DIV-162).
    // core 61 -> 62: +1 for `nulterminator` (P6 stack-array terminator
    // absorption, DIV-173).
...
#[test]
fn option_values_live_value_present_for_76() {
    let ov = OptionValues::default();
    ...
    // DIV-170).
    // 75 -> 76: +1 for `nulterminator` (live_field = nul_terminator, DIV-173).
