...
fn surface_count_is_115() {
    ...
    assert_eq!(kuna_num_surfaces(), 115);
    assert_eq!(SURFACE_TABLE.len(), 115);
}

#[test]
fn settable_count_is_187() {
    ...
    assert_eq!(kuna_num_settables(), 187);
    assert_eq!(SETTABLE_TABLE.len(), 187);
}

#[test]
fn tier_counts_are_59_core_69_transform_59_analysis() {
    let mut core = 0;
    let mut transform = 0;
    let mut analysis = 0;
    ...
    assert_eq!((core, transform, analysis), (59, 69, 59));
}
...
#[test]
fn option_values_live_value_present_for_70_suppressed_for_102() {
    let ov = OptionValues::default();
