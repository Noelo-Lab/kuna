...
fn settable_count_is_231() {
    ...
    assert_eq!(kuna_num_settables(), 231);
    assert_eq!(SETTABLE_TABLE.len(), 231);
}

#[test]
fn tier_counts_are_73_core_94_transform_64_analysis() {
    let mut core = 0;
    let mut transform = 0;
    let mut analysis = 0;
    ...
    assert_eq!((core, transform, analysis), (73, 94, 64));
}
...
fn option_values_live_value_present_for_97() {
    ...
    // 96 -> 97: +1 for `castsign` (live_field = cast_sign).
