...
fn settable_count_is_216() {
    ...
    assert_eq!(kuna_num_settables(), 216);
    assert_eq!(SETTABLE_TABLE.len(), 216);
}

#[test]
fn tier_counts_are_68_core_85_transform_63_analysis() {
    let mut core = 0;
    let mut transform = 0;
    let mut analysis = 0;
    ...
    // pointer parameter).
    assert_eq!((core, transform, analysis), (68, 85, 63));
}
