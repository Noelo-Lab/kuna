...
fn settable_count_is_104() {
    ...
    // +1 for `almostregion` (S8 latent-region inline-candidate reporting).
    assert_eq!(kuna_num_settables(), 104);
    assert_eq!(SETTABLE_TABLE.len(), 104);
}

#[test]
fn tier_counts_are_22_core_45_transform_37_analysis() {
    let mut core = 0;
    let mut transform = 0;
    let mut analysis = 0;
    ...
    // analysis 36 -> 37: +1 for `almostregion` (S8 inline-candidate reporting;
    // report-only, so it is analysis tier, not transform).
    assert_eq!((core, transform, analysis), (22, 45, 37));
}
...
fn emit_catalog_json_static_form_brackets_and_commas() {
    ...
    // not move the tail; almostregion's S8 row is appended, so it does).
