...
fn settable_count_is_38() {
    // 24 stage-model knobs (incl. `switchguardbound`, angr
    // test_decompiling_missing_function_call) + 14 analysis-tier gates: 10 per-run
    // analysis-pass enablement (noreturn_known/libproto/strings/entry_disc/
    // arm_markers/mips_gp/mips_isa/dwarf/callfixup/addrtable) + the `formatstring`
    ...
    // consumer gate + the `gopclntab` Go pclntab function-name recovery gate.
...
#[test]
fn option_values_live_value_present_for_21_suppressed_for_15() {
    let ov = OptionValues::default();
    // 21 options have a codegen live reader (realtypes joins the field-backed
    // group; switchguardbound is field-backed via switch_guard_bound); the
