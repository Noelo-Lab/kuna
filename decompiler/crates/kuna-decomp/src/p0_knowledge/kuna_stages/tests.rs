...
fn settable_count_is_38() {
    // 23 stage-model knobs + 15 analysis-tier gates: 10 per-run analysis-pass
    ...
    // gopclntab with Go pclntab name recovery, Increment 34; noreturn_propagate
    // with the angr-style no-return propagation consumer.)
...
#[test]
fn option_values_live_value_present_for_20_suppressed_for_16() {
    let ov = OptionValues::default();
    ...
    // loweredswitch/stackguard/namestyle PLUS the 15 analysis-tier gates (which
