...
//===========================================================================
// TEMPORARY INSTRUMENTATION (throwaway) — KUNA_DBG_FCR=1
//===========================================================================

/// Why `call_output_foldable` declined (debug only).
pub fn fold_reason(data: &Funcdata, vn: VarnodeId) -> String {
    let use_op = match data.lone_descend(vn) {
        Some(o) => o,
        None => {
            let n = data
                .vbank()
                .get(vn)
                .map(|v| v.descend_iter().count())
                .unwrap_or(0);
            return format!("no-lone-descend(n={n})");
        }
    };
    if op_is_marker(data, use_op) {
        return "marker-use".into();
    }
    let def = match vn_def(data, vn) {
        Some(d) => d,
        None => return "no-def".into(),
    };
    let def_op = match data.obank().get(def) {
        Some(op) => op,
        None => return "stale-def".into(),
    };
    if !def_op.is_call() || (def_op.code() == OpCode::CPUI_NEW && def_op.num_input() == 1) {
        return "not-call".into();
    }
    let def_blk = match op_parent(data, def) {
        Some(b) => b,
        None => return "no-def-blk".into(),
    };
    let use_blk = match op_parent(data, use_op) {
        Some(b) => b,
        None => return "no-use-blk".into(),
    };
    if def_blk != use_blk {
        return "cross-block".into();
    }
    let ops = data.bb_ops(def_blk);
    let def_idx = match ops.iter().position(|&o| o == def) {
        Some(i) => i,
        None => return "def-not-in-blk".into(),
    };
    let use_idx = match ops.iter().position(|&o| o == use_op) {
        Some(i) => i,
        None => return "use-not-in-blk".into(),
    };
    if use_idx <= def_idx {
        return "use-before-def".into();
    }
    for &mid in &ops[def_idx + 1..use_idx] {
        if op_is_barrier(data, mid) {
            let c = data.obank().get(mid).map(|o| o.code()).unwrap();
            return format!("barrier:{c:?}");
        }
        if op_reads_indirect_output_of(data, mid, def) {
            return "indirect-read".into();
        }
    }
    "OK".into()
}
