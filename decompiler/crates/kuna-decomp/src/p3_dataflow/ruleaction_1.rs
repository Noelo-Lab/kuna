...
impl RuleEarlyRemoval {
    /// Rule `RuleEarlyRemoval` (name "earlyremoval").
...
impl Rule for RuleEarlyRemoval {
    ...
    fn apply_op(&mut self, op: OpId, data: &mut Funcdata) -> int4 {
        // Functions automatically consumed
...
impl RuleCollectTerms {
    /// Rule `RuleCollectTerms` (name "collect_terms").
...
impl RuleSelectCse {
    /// Rule `RuleSelectCse` (name "selectcse").
...
impl Rule for RuleSelectCse {
    ...
    fn get_op_list(&self) -> Vec<OpCode> {
        // For division optimization corrections
...
impl RulePiece2Zext {
    /// Rule `RulePiece2Zext` (name "piece2zext").
...
impl Rule for RulePiece2Zext {
    ...
    fn apply_op(&mut self, op: OpId, data: &mut Funcdata) -> int4 {
        // Constant must be most significant bits
...
impl RulePiece2Sext {
    /// Rule `RulePiece2Sext` (name "piece2sext").
...
impl RuleBxor2NotEqual {
    /// Rule `RuleBxor2NotEqual` (name "bxor2notequal").
...
impl RuleOrConsume {
    /// Rule `RuleOrConsume` (name "orconsume").
...
impl RuleOrCollapse {
    /// Rule `RuleOrCollapse` (name "orcollapse").
...
impl Rule for RuleOrCollapse {
    ...
    fn apply_op(&mut self, op: OpId, data: &mut Funcdata) -> int4 {
        ...
        // first param may turn on other bits
        if (mask | val) != val {
            return 0;
        }
...
impl RuleAndOrLump {
    /// Rule `RuleAndOrLump` (name "andorlump").
...
impl RuleNegateIdentity {
    /// Rule `RuleNegateIdentity` (name "negateidentity").
...
impl RuleShiftBitops {
    /// Rule `RuleShiftBitops` (name "shiftbitops").
...
impl Rule for RuleShiftBitops {
    ...
    fn apply_op(&mut self, op: OpId, data: &mut Funcdata) -> int4 {
        ...
        // FIXME: Can't exceed uintb precision
...
impl RuleRightShiftAnd {
    /// Rule `RuleRightShiftAnd` (name "rightshiftand").
...
impl RuleIntLessEqual {
    /// Rule `RuleIntLessEqual` (name "intlessequal").
...
impl RuleEquality {
    /// Rule `RuleEquality` (name "equality").
...
impl RuleTermOrder {
    /// Rule `RuleTermOrder` (name "termorder").
...
impl RulePullsubMulti {
    /// Rule `RulePullsubMulti` (name "pullsub_multi").
...
impl Rule for RulePullsubMulti {
    ...
    fn apply_op(&mut self, op: OpId, data: &mut Funcdata) -> int4 {
        ...
        // Don't pull apart double precision
                ...
                if min_byte == 0 && in_written {
                    ...
                    if opc == OpCode::CPUI_INT_ZEXT || opc == OpCode::CPUI_INT_SEXT {
                        ...
                        if new_size == def_in0_size {
                            // We have matching extension, so new SUBPIECE will cancel anyway
                            continue;
                        }
                    }
                }
...
impl RulePullsubIndirect {
    /// Rule `RulePullsubIndirect` (name "pullsub_indirect").
...
impl RulePushMulti {
    /// Rule `RulePushMulti` (name "push_multi").
...
impl Rule for RulePushMulti {
    ...
    fn apply_op(&mut self, op: OpId, data: &mut Funcdata) -> int4 {
        ...
        // C++ binds outvn here; already bound above.

        // Move MULTIEQUAL output to op1
        ...
        if res == 1 {
            ...
            // Complete move of unified op into merge block
            data.op_insert_after(op1, substitute);
        } else {
            // Complete move of unified op into merge block
            data.op_insert_begin(op1, bl);
        }
        // Destroy the MULTIEQUAL
        data.op_destroy(op);
        // Remove the duplicate (in favor of the unified)
        data.op_destroy(op2);
        1
    }
}
...
impl RuleNotDistribute {
    /// Rule `RuleNotDistribute` (name "notdistribute").
...
impl RuleHighOrderAnd {
    /// Rule `RuleHighOrderAnd` (name "highorderand").
    ...
    fn apply_op(&mut self, op: OpId, data: &mut Funcdata) -> int4 {
        ...
        if data.vbank().get(cvn2).expect("RuleHighOrderAnd: stale cvn2").is_constant() {
            ...
            for i in 0..2 {
                ...
                // zerovn must be unaffected by the AND operation
...
impl RuleAndDistribute {
    /// Rule `RuleAndDistribute` (name "anddistribute").
...
impl RuleRangeMeld {
    /// Rule `RuleRangeMeld` (name "rangemeld").
