...
impl Architecture {
    ...
    pub fn reset_defaults_internal(&mut self) {
        ...
        self.duplicate_shared_returns = true; // (kuna) DIV-18 default-on (angr SAILR gotoless ReturnDuplicatorHigh, decbench F4 returndup): duplicate a shared bare-epilogue RETURN into each predecessor so O0 early-return guard shapes recover. Per-test opt-out (`option returndup off`) on the datatests it changes keeps the corpus byte-identical.
