...
impl Architecture {
    ...
    pub fn reset_defaults_internal(&mut self) {
        ...
        self.byte_honest = true; // (kuna) default-on: JSON-surface only (no p-code, no emitted C), so the 675-assertion datatest corpus cannot observe it; measured +8 type_match-perfect / 21 improved / 0 regressed over 1,390 scored coreutils functions
