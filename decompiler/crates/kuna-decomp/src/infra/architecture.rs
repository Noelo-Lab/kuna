...
pub struct Architecture {
    ...
    /// `funcboundflow`. Only procedures holding no other named start and no other
    /// procedure's start are used, and an entry AT a procedure start is always
    /// kept. Applied only while `pdb` is also on; off restores the previous
