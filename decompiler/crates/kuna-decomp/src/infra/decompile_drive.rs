...
impl FlowEnvironment for ArchFlowEnv {
    ...
    fn is_tail_call_branch(&self, fd: &Funcdata, op: crate::seams::OpId, dest: &Address) -> bool {
        ...
        // (`option tailcalljump on|off`, default off / default-pipeline byte-
        // identical).  The callee resolution is `query_call(dest).is_some()` (is
        // `dest` a known function entry, incl. a PLT thunk?) and the self-entry
        // check is `dest == fd.getAddress()`.
        let arch = self.arch();
