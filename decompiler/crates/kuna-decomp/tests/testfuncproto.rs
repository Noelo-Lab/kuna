...
//! `glb->translate->getRegister("r12")`, register lookup against a spec-loaded
//! `Translate`.  `assignParameterStorage` / `deriveInputMap` themselves ARE
//! ported (`fspec.rs`).  Each test carries an inline `// DEFER(W8)`.
