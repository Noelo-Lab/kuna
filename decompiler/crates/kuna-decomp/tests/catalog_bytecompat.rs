...
#[test]
fn fixture_has_all_216_settables() {
    ...
    assert_eq!(FIXTURE.matches("\"option\": ").count(), 216);
    // Every row carries the tier field appended after change_kind.
    assert_eq!(FIXTURE.matches("\"tier\": ").count(), 216);
    // ... and the symptoms array appended after tier (C3).
    assert_eq!(FIXTURE.matches("\"symptoms\": ").count(), 216);
}
