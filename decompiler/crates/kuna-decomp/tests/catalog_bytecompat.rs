...
#[test]
fn fixture_has_all_89_settables() {
    ...
    assert_eq!(FIXTURE.matches("\"option\": ").count(), 89);
    // Every row carries the tier field appended after change_kind.
    assert_eq!(FIXTURE.matches("\"tier\": ").count(), 89);
    // ... and the symptoms array appended after tier (C3).
    assert_eq!(FIXTURE.matches("\"symptoms\": ").count(), 89);
}
