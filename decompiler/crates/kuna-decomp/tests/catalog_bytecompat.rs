...
#[test]
fn fixture_has_all_35_settables() {
    // One `"option":` per settable row: 23 stage-model knobs + 12 analysis-tier
    ...
    // `formatstring` DecompilerDependent varargs-typing gate and the `listing`
    // Listing/xref disassembly tier gate).
    assert_eq!(FIXTURE.matches("\"option\": ").count(), 35);
}
