...
fn fixture_has_all_37_settables() {
    // One `"option":` per settable row: 24 stage-model knobs (incl. the
    // `switchguardbound` CBRANCH-guard jumptable bound, angr
    ...
    // consumer gate, and the `gopclntab` Go pclntab function-name recovery gate).
