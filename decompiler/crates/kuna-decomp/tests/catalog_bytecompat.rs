...
#[test]
fn fixture_has_all_212_settables() {
    ...
    assert_eq!(FIXTURE.matches("\"option\": ").count(), 212);
    // Every row carries the tier field appended after change_kind.
    assert_eq!(FIXTURE.matches("\"tier\": ").count(), 212);
    // ... and the symptoms array appended after tier (C3).
    assert_eq!(FIXTURE.matches("\"symptoms\": ").count(), 212);
}
