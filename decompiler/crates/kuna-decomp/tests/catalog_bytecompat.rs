...
fn fixture_has_all_38_settables() {
    // One `"option":` per settable row: 23 stage-model knobs + 15 analysis-tier
    ...
    // no-return Listing consumer gate, the `noreturn_propagate` no-return
    // propagation Listing consumer gate, and the `gopclntab` Go pclntab
