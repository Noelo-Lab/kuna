...
#[test]
fn fixture_has_all_50_settables() {
    ...
    // + 16 analysis-tier gates (11 per-run
    // analysis-pass enablement — including the `mips_gp` $gp recovery gate, the
    // `mips_isa` MIPS16 ISA_MODE painting gate, and the `eh_frame_full` `.eh_frame`
    // LSDA landing-pad discovery gate (GccExceptionAnalyzer, default-off) — plus
    // the `formatstring`
    ...
    assert_eq!(FIXTURE.matches("\"option\": ").count(), 50);
}
