...
#[test]
fn fixture_has_all_178_settables() {
    ...
    assert_eq!(FIXTURE.matches("\"option\": ").count(), 178);
    // Every row carries the tier field appended after change_kind.
    assert_eq!(FIXTURE.matches("\"tier\": ").count(), 178);
    // ... and the symptoms array appended after tier (C3).
    assert_eq!(FIXTURE.matches("\"symptoms\": ").count(), 178);
}
