...
//!
//! For every `tests/datatests/*.xml`, bootstrap the full XML frontend,
...
/// v1 + 1)` in `tests/datatests/forloop1.xml` (the C++ decomp_test_dbg
...
fn verify_w10_loop_emitter_renders_real_loop_on_oracle_loop_fn() {
    let path = repo_root().join("tests/datatests/forloop1.xml");
...
fn verify_w10_symbol_naming_readstruct_body_uses_param_names_not_registers() {
    let path = repo_root().join("tests/datatests/nestedoffset.xml");
...
fn verify_w10_pspec_context_loopcomment_lifts_64bit_and_structures() {
    let path = repo_root().join("tests/datatests/loopcomment.xml");
...
fn verify_w10_pspec_context_forloop_varused_lifts_64bit_and_structures() {
    let path = repo_root().join("tests/datatests/forloop_varused.xml");
