...
fn w8_fw_universalaction_allgroups_full_order_count_head_tail() {
    ...
    assert_eq!(
        nonblank, 272,
        "full universal tree must render 252 C++ leaves + 13 kuna leaves (branchflip + almostregion + gotoreduce + taildup + ifelseflatten + crossjumprevert + dedupitetail + returndup + iteregion + iteboolean + earlyreturn + switchreturn + paramcopyhoist) + 7 container headers"
    );
