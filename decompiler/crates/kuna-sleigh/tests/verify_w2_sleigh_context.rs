...
#[test]
fn context_snapshot_retains_change_masks_and_tracked_registers() {
    let manager = test_manager();
    let mut db = ContextInternal::new();
    db.register_variable(b"mode", 0, 7).unwrap();
    db.set_variable_default(b"mode", 3).unwrap();
    db.set_variable(b"mode", &addr(&manager, 0x100), 1).unwrap();
    db.set_variable(b"mode", &addr(&manager, 0x200), 2).unwrap();
    ...
    db.get_tracked_default().push(tracked.clone());
    db.create_set(&addr(&manager, 0x100), &addr(&manager, 0x200))
        .push(tracked);
    let mut snapshot = db.snapshot();
    assert_eq!(snapshot.get_default_value(), db.get_default_value());
    assert_eq!(
        snapshot.get_tracked_set(&addr(&manager, 0x150)),
        db.get_tracked_set(&addr(&manager, 0x150))
    );
    assert_eq!(snapshot.get_tracked_default(), db.get_tracked_default());
    snapshot
        .set_variable(b"mode", &addr(&manager, 0x80), 4)
        .unwrap();
    snapshot
        .set_variable(b"mode", &addr(&manager, 0x180), 5)
        .unwrap();
    snapshot.get_tracked_default().clear();
    assert_eq!(
        snapshot
            .get_variable_value(b"mode", &addr(&manager, 0x100))
            .unwrap(),
        1
    );
    assert_eq!(
        snapshot
            .get_variable_value(b"mode", &addr(&manager, 0x200))
            .unwrap(),
        2
    );
    assert_eq!(get(&db, &manager, b"mode", 0x80), 3);
    assert_eq!(get(&db, &manager, b"mode", 0x180), 1);
    assert_eq!(db.get_tracked_default().len(), 1);
}

#[test]
fn temporary_sleigh_context_restores_after_return_and_unwind() {
    use kuna_sleigh::loadimage::RawLoadImage;
    ...
    db.register_variable(b"mode", 0, 7).unwrap();
    db.set_variable_default(b"mode", 3).unwrap();
    let engine = Sleigh::new(Box::new(RawLoadImage::new("synthetic")), Box::new(db));
    for unwind in [false, true] {
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            engine.with_temporary_context(|| {
                engine.with_context_db_mut(|db| {
                    db.set_variable_default(b"mode", 4).unwrap();
                    db.set_variable(b"mode", &addr(&manager, 0x100), 1).unwrap();
                });
                engine.with_temporary_context(|| {
                    engine
                        .with_context_db_mut(|db| db.set_variable_default(b"mode", 5).unwrap());
                });
                engine.with_context_db_mut(|db| {
                    assert_eq!(db.get_default_value_by_name(b"mode").unwrap(), 4)
                });
                if unwind {
        ...
        engine.with_context_db_mut(|db| {
            assert_eq!(db.get_default_value_by_name(b"mode").unwrap(), 3);
            assert_eq!(
                db.get_variable_value(b"mode", &addr(&manager, 0x100))
                    .unwrap(),
                3
            );
            let (_, first, last) = db.get_context_bounds(&addr(&manager, 0x100));
            assert_eq!((first, last), (0, u32::MAX as u64));
        });
    }
}
