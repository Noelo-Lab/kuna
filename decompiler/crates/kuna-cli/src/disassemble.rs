...
//! the only place code can resume (`kuna_console::disasm`).
//!
...
//! that no row of it starts at ([`kuna_console::disasm::skipped_targets`]), because a decode a byte
//! out of phase is spelled exactly like one that is not. And `--follow` decodes
//! from those addresses as well as from the start (`kuna_console::disasm`), filling
...
use kuna_console::disasm::{
    self, hex, mapped_run_end, read_upto, CodeWalk, Row, Window, DERIVED_INSTRUCTION_CAP,
};
...
pub(crate) struct DisArgs {
    ...
    /// only from one end (`kuna_console::disasm::walk`).
...
impl Region {
    /// The walker's view of this region ([`kuna_console::disasm::walk`]).
    fn window(&self) -> Window {
        Window { start: self.start, end: self.end, derived: self.derived, mapped_end: self.mapped_end }
    }
    ...
    let text = match view {
        View::Code => {
            let CodeWalk { rows, truncated, folded, skipped, anchored } =
                disasm::walk(prog, &region.window(), args.count, args.follow);
            if folded > 0 {
...
///
/// Walked by [`kuna_console::disasm::walk`], so it is the same decode this command lists and an
...
pub(crate) fn function_listing(prog: &ConsoleProgram, start: u64, end: u64) -> Option<String> {
    ...
    let CodeWalk { rows, .. } = disasm::walk(prog, &region.window(), None, false);
    if rows.is_empty() {
        return None;
    }
    let mut out = String::new();
