        ...
        for name,addr,txt in CC.split_functions(src):
            ...
            for i in range(n):
                ...
                if nt.kind=="id" and (k+1>=n or toks[k+1].text not in ("(",".","->","[")):
                    d=decl.get(nt.text)
                    if d is not None and d==ty:
                        red+=1; byty[ty]+=1
                        if ty not in ex:
                            ex[ty]=(f"{f.parts[-3]}/{f.stem}@{hex(addr)}",
                                    "".join(t.text for t in toks[i:k+1]), nt.text, d)
    return tot,red,byty,ex
for root in sys.argv[1:]:
    tot,red,byty,ex=run(root)
    print(f"### {root}: {red} redundant of {tot} casts ({100.0*red/tot:.1f}%)")
    for ty,n in byty.most_common(8):
        e=ex[ty]
        print(f"    {ty:<14}{n:>6}   e.g. {e[1]}  with `{e[3]} {e[2]};`   [{e[0]}]")
