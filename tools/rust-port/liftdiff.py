...
The differential itself lives in Rust:
  ...
  - `rust/crates/kuna-harness/tests/lift_diff.rs` is the gate (>= 8 of the pinned
    corpus cases structurally equal end-to-end; every divergence/exclusion is
    attributed to a named unported pre-heritage action or a W4 flow seam).

This script is a thin convenience wrapper with two subcommands:
...
# `rust/crates/kuna-harness/tests/lift_diff.rs`).
CASES = [
