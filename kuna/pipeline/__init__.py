"""kuna.pipeline -- the continuous, reference-decompiler-inspired feature loop.
...
The reference side is abstracted (`kuna.pipeline.reference`) so angr can later be joined
or replaced by reko / Binary Ninja / IDA without touching compare/sweep/worker.

Config is environment-driven (see `kuna.pipeline.config`); the defaults target this
